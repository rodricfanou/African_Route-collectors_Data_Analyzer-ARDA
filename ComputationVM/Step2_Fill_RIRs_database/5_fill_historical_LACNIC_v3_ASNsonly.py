...
## Store ASNs allocated by LACNIC to ISPs or any organization in its region
...
for folder in folders:
    ...
    ### Build a check list
    sql_command = """select * from ASNs_LACNIC ;"""
    ...
    for filei in list_of_files:
        if os.path.exists(filei) and os.path.isfile(filei):
                    ...
                    if line != '' and 'asn' in line and '*' not in line and 'summary' not in line:
                        try:
                            ...
                            if len(db_dump2) == 0:
                                ...
                                sql_commandb = """ INSERT INTO ASNs_LACNIC (ASN, CC, date, Status) VALUES (%s,%s,%s,%s);"""
            ...
            with open('record_files_parsed_by_5_fill_historical_LACNIC_v3_ASNsonly.txt', 'a') as fg:
                fg.write('%s; %s\n ' % (filei, k_insertion))
