...
for folder in folders:
    ...
    ### Build a check list
    sql_command = """select * from ASNs_ARIN ;"""
	...
	if os.path.exists(filei) and os.path.isfile(filei):
	    ...
	    k_insertion = 0
            print ' \nASN parser: We are in folder', folder , 'file', filei, 'which is the num', list_of_files.index(filei)
			...
			try:
				...
				value1 = ASNf+ '_' + CCf + '_' + Datef + '_' + Statusf
                        	...
                        	if value1 not in Check_list:
                            		...
                            		sql_commandb = """ INSERT INTO ASNs_ARIN (ASN, CC, date, Status) VALUES (%s,%s,%s,%s);"""
