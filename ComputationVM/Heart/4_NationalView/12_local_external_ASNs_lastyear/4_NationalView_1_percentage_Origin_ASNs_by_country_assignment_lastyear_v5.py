...
## Create a logfile:
name_log_file = 'Log_' + str(now_datetime) + '_' + '1_Number_Origin_ASNs_visible_at_an_IXP_lastyear' + '.txt'
...
if len(data) > 0:
    while (i < len(data)):
        ...
        if int(asn) not in CC_ASNs_AFRINIC[current_CC]:
            CC_ASNs_AFRINIC[current_CC].append(int(asn))
...
if List_all_tables > 0:
    ...
    for ixp in list(IXP_collector.keys()):
        ...
        if Current_country_ixp not in list(month_ASN.keys()):
            month_ASN[Current_country_ixp] = {}
                        ...
                        while (i < len(data)):
                            ...
                            for OriginASNs_elmt in OriginASNs:
                                ...
                                if (year, month) in lastYearList:
                                    ...
                                    if str(month) + '-' + str(year) not in list(month_ASN[Current_country_ixp].keys()):
                                        month_ASN[Current_country_ixp][str(month) + '-' + str(year)] = []

                                    if OriginASNs_elmt not in month_ASN[Current_country_ixp][
                                        str(month) + '-' + str(year)]:
                                        month_ASN[Current_country_ixp][str(month) + '-' + str(year)].append(
                                            OriginASNs_elmt)
...
for Current_country_ixp in list(month_ASN.keys()):

    for month_year_current in list(month_ASN[Current_country_ixp].keys()):

        tab = month_year_current.split('-')

        IXP_OriginASes = []

        IXP_OriginASes = copy.deepcopy(month_ASN[Current_country_ixp][month_year_current])
        ...
        for CC in list(CC_IXP.keys()):
            ...
            if Current_country_ixp == CC:
                ...
                with open(filename1, 'a') as fg:
                    fg.write('%s; %s; %s; %s\n' % (
                    month_year_current, 'Private ASNs', len(ori_dict['PRIVATE']), percentage_private))
                ...
                with open(filename1, 'a') as fg:
                    fg.write('%s;%s;%s;%s\n' % (
                    month_year_current, 'Reserved ASNs', len(ori_dict['RESERVED']), percentage_reserved))
                ...
                with open(filename1, 'a') as fg:
                    fg.write('%s;%s;%s;%s\n' % (month_year_current, 'External AFRINIC ASNs', total1, total))
