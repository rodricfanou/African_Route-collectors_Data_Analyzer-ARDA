...
if List_all_tables > 0:
                    ...
                    for ixp in list(IXP_collector.keys()):  # delete de 2 for the complete continent
                        ...
                        for route_collector in IXP_collector[ixp]:  # delete de 2 for the complete continent
                            ...
                            if str(route_collector) == str(route_collector_extracted):
                                ...
                                if CC not in list(Counter.keys()):
                                    Counter[CC] = {}

                                if IPversion not in list(Counter[CC].keys()):
    ...
    for IPversion in list(CC_prefixes_AFRINIC.keys()):
        ...
        for CC_AF in list(CC_prefixes_AFRINIC[IPversion].keys()):
            ...
            for prefix_adv in list(set(CC_prefixes_AFRINIC[IPversion][CC_AF])):
                ...
                if CC_AF in list(Counter.keys()):

                    if CC_AF not in list(Intersection[IPversion].keys()):
                        Intersection[IPversion][CC_AF] = []

                    if IPversion in list(Counter[CC_AF].keys()):
    ...
    for IPversion in list(Intersection.keys()):
        ...
        with open(filename, 'a') as fg:
            ...
            fg.write('%s; %s; %s; %s;  %s\n' % ('IPversion', 'CC_AF', 'percentage_IPv4_blocks_appearing_at_CC',
                                                'len(Intersection_IPversion_blocks_appearing)',
                                                'len(IPversion_blocks_assigned_to_country)'))

        for CC_AF in list(Intersection[IPversion].keys()):
            ...
            with open(filename, 'a') as fg:
                ...
                try:
                    perc_IP_blocks = 100 * (float(len(list(set(Intersection[IPversion][CC_AF]))))) / (
                        float(len(list(set(CC_prefixes_AFRINIC[IPversion][CC_AF])))))
                except:
                    perc_IP_blocks = 0

                fg.write('%s; %s; %s; %s; %s\n' % (
                IPversion, CC_AF, perc_IP_blocks, len(list(set(Intersection[IPversion][CC_AF]))),
