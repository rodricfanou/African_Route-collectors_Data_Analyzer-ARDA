...
if len(List_all_tables) > 0:
    ...
    for ixp in IXP_collector.keys():
        ...
        if Current_country_ixp not in year_ASN.keys():
            year_ASN[Current_country_ixp] = {}

        for year in yearList:

            if year not in year_ASN[Current_country_ixp].keys():
...
for Current_country_ixp in year_ASN.keys():
    ...
    for year_current in year_ASN[Current_country_ixp].keys():
