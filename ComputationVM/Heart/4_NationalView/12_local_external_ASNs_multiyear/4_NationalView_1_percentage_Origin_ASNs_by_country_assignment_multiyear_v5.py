...
## Create a logfile:
name_log_file = 'Log_' + str(now_datetime) + '_' + '1_Number_Origin_ASNs_visible_at_an_IXP_multiyear.txt'
...
output_folder = '../../Computation_outputs_National_View/12_local_external_ASNs_multiyear/'
...
if len(data) > 0:
    while (i < len(data)):
        ...
        if current_CC not in list(CC_ASNs_AFRINIC.keys()) and current_CC != '':
...
if len(List_all_tables) > 0:
    ...
    for ixp in list(IXP_collector.keys()):
        ...
        if Current_country_ixp not in list(year_ASN.keys()):
            year_ASN[Current_country_ixp] = {}

        for year in yearList:

            if year not in list(year_ASN[Current_country_ixp].keys()):
                year_ASN[Current_country_ixp][year] = []
            ...
            for month in range(1, 13):
                ...
                if 'Data__' + str(year) + "_" + str(month) in List_all_tables:
                    ...
                    log_file_instance.write(
                        str(now_datetime) + 'Last year Fetching data from IXP ' + Current_country_ixp + '\n')
                    ...
                    if len(data) > 0:
                        ...
                        while (i < len(data)):
                            ...
                            for OriginASNs_elmt in OriginASNs:
                                ...
                                if OriginASNs_elmt not in year_ASN[Current_country_ixp][year]:
                                    year_ASN[Current_country_ixp][year].append(OriginASNs_elmt)

                            i += 1

                    else:
                        print('No prefix found for ', Current_country_ixp, ' in ',
                              'Data__' + str(year) + '_' + str(month))
...
for Current_country_ixp in list(year_ASN.keys()):

    filename = output_folder + 'Percentage_Origin_ASNs_by_country_assignment_' + Current_country_ixp + '.txt'
    filename1 = output_folder + 'Percentage_Origin_ASNs_by_region_' + Current_country_ixp + '.txt'

    with open(filename1, 'a') as fg:
        fg.write('%s;%s;%s;%s\n' % ('##month_year', 'Type of ASNs (Region)', 'len_ASNs_type', 'percentage_type'))

    for year_current in list(year_ASN[Current_country_ixp].keys()):

        print('A last check on the CC_ASNs_AFRINIC we have ', CC_ASNs_AFRINIC)

        IXP_OriginASes = []

        IXP_OriginASes = copy.deepcopy(year_ASN[Current_country_ixp][year_current])
        ...
        for CC in list(CC_IXP.keys()):
            ...
            if Current_country_ixp == CC:
                ...
                if len(ori_dict['PRIVATE']) > 0:
                    current_filename = output_folder + 'files_origin/Private_ASNs_' + Current_country_ixp + '.txt'
                ...
                if len(ori_dict['RESERVED']) > 0:
                    current_filename = output_folder + 'files_origin/Reserved_ASNs_' + Current_country_ixp + '.txt'
                ...
                ## Local
                if current_CC in list(CC_ASNs_AFRINIC.keys()):
                ...
                if len(intersection) > 0:
                    current_filename = output_folder + 'files_origin/Local_AFRINIC_ASNs_' + Current_country_ixp + '.txt'
                ...
                for cc_external in list(CC_ASNs_AFRINIC.keys()):
                    ...
                    if current_CC != cc_external:
                        if cc_external not in list(ori_dict['external_AFRINIC'].keys()):
                            ...
                            if cc_external in list(CC_ASNs_AFRINIC.keys()):
                                intersection2 = list(set(IXP_OriginASes) & set(CC_ASNs_AFRINIC[cc_external]))

                            if len(intersection2) > 0:
                                current_filename = output_folder + 'files_origin/External_AFRINIC_ASNs_' + Current_country_ixp + '.txt'
                ...
                for cc_region in CC_ASNs_RIPE:
                    ...
                    if current_CC != cc_region:
                        if cc_region not in list(ori_dict['RIPE'].keys()):
                            ...
                            if cc_region in list(CC_ASNs_RIPE.keys()):
                                intersection3 = list(set(IXP_OriginASes) & set(CC_ASNs_RIPE[cc_region]))

                            if len(intersection3) > 0:
                                current_filename = output_folder + 'files_origin/RIPE_ASNs_' + Current_country_ixp + '.txt'
                ...
                for cc_region in CC_ASNs_ARIN:
                    ...
                    if current_CC != cc_region:
                        if cc_region not in list(ori_dict['ARIN'].keys()):
                            ...
                            if cc_region in list(CC_ASNs_ARIN.keys()):
                                intersection5 = list(set(IXP_OriginASes) & set(CC_ASNs_ARIN[cc_region]))

                            if len(intersection5) > 0:
                                current_filename = output_folder + 'files_origin/ARIN_ASNs_' + Current_country_ixp + '.txt'
                ...
                for cc_region in CC_ASNs_APNIC:
                    ...
                    if current_CC != cc_region:
                        if cc_region not in list(ori_dict['APNIC'].keys()):
                            ...
                            if cc_region in list(CC_ASNs_APNIC.keys()):
                                intersection4 = list(set(IXP_OriginASes) & set(CC_ASNs_APNIC[cc_region]))

                            if len(intersection4) > 0:
                                current_filename = output_folder + 'files_origin/APNIC_ASNs_' + Current_country_ixp + '.txt'
                ...
                for cc_region in CC_ASNs_LACNIC:
                    ...
                    if current_CC != cc_region:
                        if cc_region not in list(ori_dict['LACNIC'].keys()):
                            ...
                            if cc_region in list(CC_ASNs_LACNIC.keys()):
                                intersection6 = list(set(IXP_OriginASes) & set(CC_ASNs_LACNIC[cc_region]))

                            if len(intersection6) > 0:
                                current_filename = output_folder + 'files_origin/LACNIC_ASNs_' + Current_country_ixp + '.txt'
