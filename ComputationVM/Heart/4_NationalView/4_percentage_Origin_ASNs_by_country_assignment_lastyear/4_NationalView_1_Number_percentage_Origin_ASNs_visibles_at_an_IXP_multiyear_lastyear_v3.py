...
## Create a logfile:
name_log_file = 'Log_'+str(now_datetime) + '_' + '4_NationalView_1_Number_Origin_ASNs_visibles_at_an_IXP_multiyear_lastyear' + '.txt'
...
if List_all_tables > 0:
    ...
    for ixp in IXP_collector.keys():
        ...
        if CC_key  not in year_ASN.keys():
            
            year_ASN[CC_key] = {}
        
        if CC_key  not in month_ASN.keys():
            
            month_ASN[CC_key] = {}
        
        for year in yearList:
            
            if year not in year_ASN[CC_key].keys():
                        ...
                        while (i<len(data)):
                            ...
                            for OriginASNs_elmt in OriginASNs :
                                ...
                                if (year, month) in lastYearList:
                                    ...
                                    if str(month)+'-'+str(year) not in month_ASN[CC_key].keys():
...
for ixp in IXP_collector.keys():
    ...
    if os.path.isfile(filename):
        ...
        with open(filename, 'a') as fd:
            fd.write('%s;%s;%s;%s;%s;%s\n'%('CC',  'month-Year', 'Number of Origin ASNs assigned to the country visibles at the IXP', 'Number of Origin ASNs visibles at the IXP', 'Percentage of local Origin ASNs', 'Percentage of External Origin ASNs' ))
    ...
    if os.path.isfile(filename):
        ...
        with open(filename, 'a') as fd:
            ...
            fd.write('%s;%s;%s;%s;%s;%s\n'%('CC',  'Year', 'Number of Origin ASNs assigned to the country visibles at the IXP', 'Number of Origin ASNs visibles at the IXP', 'Percentage of local Origin ASNs', 'Percentage of External Origin ASNs' ))
...
for ixp in IXP_collector.keys():
    ...
    if CC_key in month_ASN.keys():
        
        filename = output_folder+'LastYear__local_vs_External_ASNs_to_the_country_hosting_IXP_visible_at_IXP_'+CC_key+'.txt'
        
        intersection1 = []
        
        for elmt in month_ASN[CC_key].keys():
                ...
                with open(filename, 'a') as fd:
                        ...
                        try:
                            ...
                            fd.write('%s;%s;%s;%s;%s;%s\n'%(current_CC,  elmt, len(intersection1), len(month_ASN[CC_key][elmt]),  percentage_local_AF1, 100-percentage_local_AF1 ))
                        except:
                            fd.write('%s;%s;%s;%s;%s;%s\n'%(current_CC,  elmt, len(intersection1), len(month_ASN[CC_key][elmt]),  percentage_local_AF1, 100-percentage_local_AF1 ))
    ...
    if CC_key in year_ASN.keys():
        ...
        for year in yearList:
                ...
                with open(filename, 'a') as fd:
                        ...
                        try:
                            A = pycountry.countries.get(alpha2=current_CC)
                            fd.write('%s;%s;%s;%s;%s;%s\n'%(current_CC,  year, len(intersection2), len(year_ASN[CC_key][year]),  percentage_local_AF2, 100-percentage_local_AF2 ))
                        except:
                            fd.write('%s;%s;%s;%s;%s;%s\n'%(current_CC,  year, len(intersection2), len(year_ASN[CC_key][year]),  percentage_local_AF2, 100-percentage_local_AF2 ))
