...
name_log_file = 'Log_' + str(
    now_datetime) + '_' + 'RegionalView_1_Number_peering_ASNs_visible_at_an_IXP_lastyear_each_country' + '.txt'
...
output_folder = '../../Computation_outputs_National_View/1_Number_ASNs_peering_in_more_than_1_IXP_per_country_lastyear/'
...
for CC in week_ASN:
    ...
    with open(output_folder + 'LastYear__list_visible_ASNs_peering_at_IXP_' + CC + '.txt', 'a') as fh:

        fh.write('%s\n' % ("""###Month-Year; Visible ASNs peering at the IXP"""))
...
for ixp in list(IXP_collector.keys()):
    ...
    if os.path.isfile(
            output_folder + 'LastYear__2bytes_list_peering_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt'):

        create_output_2bytesASN_list = open(
            output_folder + 'LastYear__2bytes_list_peering_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt', 'a')
        ...
        create_output_2bytesASN_list.write('###Month-Year; Visible 2bytes ASNs peering at more than 1 IXP \n')
...
for ixp in list(IXP_collector.keys()):
    ...
    if os.path.isfile(
            output_folder + 'LastYear__4bytes_list_peering_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt'):

        create_output_4bytesASN_list = open(
            output_folder + 'LastYear__4bytes_list_peering_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt', 'a')
        ...
        create_output_4bytesASN_list.write('###Month-Year; Visible 2bytes ASNs peering at more than 1 IXP \n')
...
for CC in week_ASN:
    ...
    with open(output_folder + 'LastYear__number_visible_peering_ASNs_in_more_than_1_IXP_of_' + CC + '.txt', 'a') as fg:

        fg.write('%s\n' % (
            """###Month-Year; Number of Visible peering ASNs; Number of Visible peering 2bytes ASNs; Number of Visible peering 4bytes ASNs;"""))
