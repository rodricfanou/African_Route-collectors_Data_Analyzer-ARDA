...
name_log_file = 'Log_' + str(
    now_datetime) + '_' + 'RegionalView_1_Number_origin_ASNs_visible_at_an_IXP_lastyear_each_country' + '.txt'
...
for CC in week_ASN:
    ...
    with open(output_folder + 'LastYear__list_visible_origin_ASNs_at_IXP_' + CC + '.txt', 'a') as fh:

        fh.write('%s\n' % ("""###Month-Year; Visible Origin ASNs at the IXP"""))
...
for ixp in list(IXP_collector.keys()):
    ...
    if os.path.isfile(
            output_folder + 'LastYear__2bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt'):

        create_output_2bytesASN_list = open(
            output_folder + 'LastYear__2bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt', 'a')
        ...
        create_output_2bytesASN_list.write('###Month-Year; Visible 2bytes origin ASNs at more than 1 IXP \n')
...
for ixp in list(IXP_collector.keys()):
    ...
    if os.path.isfile(
            output_folder + 'LastYear__4bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt'):

        create_output_4bytesASN_list = open(
            output_folder + 'LastYear__4bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt', 'a')
        ...
        create_output_4bytesASN_list.write('###Month-Year; Visible 2bytes origin ASNs at more than 1 IXP \n')
...
for CC in week_ASN:
    ...
    with open(output_folder + 'LastYear__number_visible_origin_ASNs_in_more_than_1_IXP_of_' + CC + '.txt', 'a') as fg:

        fg.write('%s\n' % (
            """###Month-Year; Number of Visible Origin ASNs; Number of Visible Origin 2bytes ASNs; Number of Visible Origin 4bytes ASNs;"""))
