...
## Create a logfile:
name_log_file = 'Log_'+str(now_datetime) + '_' + 'RegionalView_1_Number_peering_ASNs_visible_at_an_IXP_lastmonth_each_country' + '.txt'
...
for ixp in IXP_collector.keys():
    ...
    if os.path.isfile (output_folder + 'LastMonth__2bytes_list_peering_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt'):
        
        create_output_2bytesASN_list = open(output_folder+'LastMonth__2bytes_list_peering_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt', 'a')
...
for ixp in IXP_collector.keys():
    ...
    if os.path.isfile (output_folder + 'LastMonth__4bytes_list_peering_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt'):
        
        create_output_4bytesASN_list = open(output_folder+'LastMonth__4bytes_list_peering_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt', 'a')

    else:
        
        create_output_4bytesASN_list = open(output_folder+'LastMonth__4bytes_list_peering_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt', 'a')
        
        create_output_4bytesASN_list.write('###Num Week; Timestamp beginning;  Timestamp end;  Datetime beginning;  Datetime end; Visible 2bytes ASNs peering at more than 1 IXP \n')
