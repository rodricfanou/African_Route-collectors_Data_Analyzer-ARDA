...
name_log_file = 'Log_' + str(
    now_datetime) + '_' + 'NationalView_1_Number_ASNs_announcing_v4_v6_lastyear_each_country' + '.txt'
...
if os.listdir(IXPView_output_folder) != []:
    ...
    for ixp in list(IXP_collector.keys()):
        ...
        filename = IXPView_output_folder + """LastYear_""" + ixp + """_List_ASNs_announcing_v4.txt"""
...
if os.listdir(IXPView_output_folder) != []:
    ...
    for ixp in list(IXP_collector.keys()):
        ...
        filename = IXPView_output_folder + """LastYear_""" + ixp + """_List_ASNs_announcing_v6.txt"""
...
for CC in week_ASN_announcing_v6:
    ...
    with open(output_folder + 'LastYear__list_ASNs_announcing_v6_at_IXPs_in_' + CC + '.txt', 'a') as fh:
...
for CC in week_ASN_announcing_v4:
    ...
    with open(output_folder + 'LastYear__list_ASNs_announcing_v4_at_IXPs_in_' + CC + '.txt', 'a') as fh:
    ...
    with open(output_folder + 'LastYear__Number_ASNs_announcing_v4_v6_at_IXPs_in_' + CC + '.txt', 'a') as fh:
    ...
    for couple_timestamp in list(week_ASN_announcing_v4[CC].keys()):
        ...
        with open(output_folder + 'LastYear__Number_ASNs_announcing_v4_v6_at_IXPs_in_' + CC + '.txt', 'a') as fh:
            ...
            if couple_timestamp not in list(week_ASN_announcing_v6[CC].keys()):
                ...
                fh.write('%s; %s; %s\n' % (couple_timestamp, len(week_ASN_announcing_v4[CC][couple_timestamp]),
                                           len(week_ASN_announcing_v6[CC][couple_timestamp])))
