...
name_log_file = 'Log_' + str(
    now_datetime) + '_' + 'RegionalView_1_Number_origin_ASNs_visible_at_an_IXP_lastmonth_each_country' + '.txt'
...
output_folder = '../../Computation_outputs_National_View/1_Number_ASNs_origin_in_more_than_1_IXP_per_country_lastmonth/'

## Update the folder with the selected repository
IXPView_output_folder = '/var/www/html/.../outputs/2_Number_Origin_ASNs_visibles_at_an_IXP_lastmonth/'
...
for CC in week_ASN:
    ...
    with open(output_folder + 'LastMonth__list_visible_ASNs_origin_at_IXP_' + CC + '.txt', 'a') as fh:

        fh.write('%s\n' % (
            """###Num Week; Timestamp beginning;  Timestamp end;  Datetime  beginning;  Datetime end; Visible ASNs origin at the IXP"""))
...
for ixp in list(IXP_collector.keys()):
    ...
    if os.path.isfile(
            output_folder + 'LastMonth__2bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt'):

        create_output_2bytesASN_list = open(
            output_folder + 'LastMonth__2bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt', 'a')
        ...
        create_output_2bytesASN_list.write(
            '###Num Week; Timestamp beginning;  Timestamp end;  Datetime beginning;  Datetime end; Visible 2bytes origin ASNs at more than 1 IXP \n')
...
for ixp in list(IXP_collector.keys()):
    ...
    if os.path.isfile(
            output_folder + 'LastMonth__4bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt'):

        create_output_4bytesASN_list = open(
            output_folder + 'LastMonth__4bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt', 'a')
        ...
        create_output_4bytesASN_list.write(
            '###Num Week; Timestamp beginning;  Timestamp end;  Datetime beginning;  Datetime end; Visible 2bytes origin ASNs at more than 1 IXP \n')
...
for CC in week_ASN:
    ...
    with open(output_folder + 'LastMonth__number_visible_origin_ASNs_in_more_than_1_IXP_of_' + CC + '.txt', 'a') as fg:

        fg.write('%s\n' % (
            """###Num Week; Timestamp beginning;  Timestamp end;  Datetime beginning;  Datetime end; Number of Visible origin ASNs; Number of Visible origin 2bytes ASNs; Number of Visible origin 4bytes ASNs;"""))

        for couple_timestamp in list(week_ASN[CC].keys()):

            # print 'len(week_ASN[CC][couple_timestamp]) = ', len(week_ASN[CC][couple_timestamp])

            # print 'len(week_ASN_2bytes[CC][couple_timestamp]) = ', len(week_ASN_2bytes[CC][couple_timestamp])

            # print 'len(week_ASN_4bytes[CC][couple_timestamp]))) = ', pprint(week_ASN_4bytes[CC])

            if couple_timestamp in list(week_ASN_4bytes[CC].keys()) and couple_timestamp in list(
                    week_ASN_2bytes[CC].keys()):

                fg.write('%s; %s; %s; %s\n' % (
                couple_timestamp, len(week_ASN[CC][couple_timestamp]), len(week_ASN_2bytes[CC][couple_timestamp]),
                len(week_ASN_4bytes[CC][couple_timestamp])))

            elif couple_timestamp not in list(week_ASN_4bytes[CC].keys()) and couple_timestamp in list(
                    week_ASN_2bytes[CC].keys()):

                fg.write('%s; %s; %s; %s\n' % (
                couple_timestamp, len(week_ASN[CC][couple_timestamp]), len(week_ASN_2bytes[CC][couple_timestamp]), '0'))

            elif couple_timestamp in list(week_ASN_4bytes[CC].keys()) and couple_timestamp not in list(
                    week_ASN_2bytes[CC].keys()):
