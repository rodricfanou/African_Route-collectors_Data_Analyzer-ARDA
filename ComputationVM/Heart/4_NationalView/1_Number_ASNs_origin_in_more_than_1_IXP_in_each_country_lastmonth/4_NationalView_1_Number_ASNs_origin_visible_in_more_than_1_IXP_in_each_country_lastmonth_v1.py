...
## Create a logfile:
name_log_file = 'Log_'+str(now_datetime) + '_' + 'RegionalView_1_Number_origin_ASNs_visible_at_an_IXP_lastmonth_each_country' + '.txt'
...
IXPView_output_folder = '/var/www/html/ARP/controleurs/scripts/ARP_visual/outputs/2_Number_Origin_ASNs_visibles_at_an_IXP_lastmonth/'
...
for ixp in IXP_collector.keys():
    ...
    if os.path.isfile (output_folder + 'LastMonth__2bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt'):
        
        create_output_2bytesASN_list = open(output_folder+'LastMonth__2bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt', 'a')
    
    else:
    
        create_output_2bytesASN_list = open(output_folder+'LastMonth__2bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt', 'a')
        
        create_output_2bytesASN_list.write('###Num Week; Timestamp beginning;  Timestamp end;  Datetime beginning;  Datetime end; Visible 2bytes origin ASNs at more than 1 IXP \n')
...
for ixp in IXP_collector.keys():
    ...
    if os.path.isfile (output_folder + 'LastMonth__4bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt'):
        
        create_output_4bytesASN_list = open(output_folder+'LastMonth__4bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt', 'a')

    else:
        
        create_output_4bytesASN_list = open(output_folder+'LastMonth__4bytes_list_origin_ASNs_in_more_than_1_IXP_in_' + CC_to_consider + '.txt', 'a')
        
        create_output_4bytesASN_list.write('###Num Week; Timestamp beginning;  Timestamp end;  Datetime beginning;  Datetime end; Visible 2bytes origin ASNs at more than 1 IXP \n')
