...
name_log_file = 'Log_' + str(
    now_datetime) + '_' + '4_NationalView_5_Number_prefixes_visible_at_an_IXP_multiyear_lastyear_better' + '.txt'
...
if len(List_all_tables) > 0:
    ...
    for ixp in list(IXP_collector.keys()):
        ...
        create_output_MultiYear = open(output_folder + 'MultiYear__list_visible_prefixes_at_IXP_' + CC_key + '.txt',
                                       'a')
...
for ixp in list(IXP_collector.keys()):
    ...
    if os.path.isfile(filename):
        ...
        with open(filename, 'a') as fd:
            ...
            'CC', 'month-Year', 'Number of prefixes assigned to the country visibles at the IXP',
            'Number of prefixes visibles at the IXP', 'Percentage of local prefixes',
            'Percentage of External prefixes'))
    ...
    if os.path.isfile(filename):
        ...
        with open(filename, 'a') as fd:
            ...
            fd.write('%s;%s;%s;%s;%s;%s\n' % (
            'CC', 'Year', 'Number of prefixes assigned to the country visibles at the IXP',
            'Number of prefixes visibles at the IXP', 'Percentage of local prefixes',
            'Percentage of External prefixes'))
            ...
            for id_month_and_year in list(month_prefix[CC_key].keys()):
                ...
                if len(month_prefix[CC_key][id_month_and_year]) > 0:
                    ...
                    try:
                        ...
                        percentage_local_AF = 100 * (
                                    float(len(intersection)) / float(len(month_prefix[CC_key][id_month_and_year])))
                    except:
                    ...
                    with open(filename, 'a') as fd:
                        ...
                        try:
                            ...
                            fd.write('%s;%s;%s;%s;%s;%s\n' % (current_CC, id_month_and_year, len(intersection),
                                                              len(month_prefix[CC_key][id_month_and_year]),
                                                              percentage_local_AF, 100 - percentage_local_AF))
            ...
            for year in yearList:
                ...
                with open(filename, 'a') as fd:
                    ...
                    try:
                        A = pycountry.countries.get(alpha2=current_CC)
                        fd.write('%s;%s;%s;%s;%s;%s\n' % (
                        current_CC, year, len(intersection), len(year_prefix[CC_key][year]), percentage_local_AF,
                        100 - percentage_local_AF))
