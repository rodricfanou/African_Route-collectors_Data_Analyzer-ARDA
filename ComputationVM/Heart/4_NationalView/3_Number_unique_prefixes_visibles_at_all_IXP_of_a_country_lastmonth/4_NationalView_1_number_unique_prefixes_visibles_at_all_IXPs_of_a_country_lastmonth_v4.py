...
while (i < len(data)):
    ...
    if row[2] not in list(week_prefix.keys()):
        week_prefix[row[2]] = {}
                ...
                for line in fg:
                    ...
                    if '#' not in line:
                        ...
                        if '/' in tab[-1]:
                            ...
                            if CC_to_consider not in list(week_prefix.keys()):
                                week_prefix[CC_to_consider] = {}

                            if couple_timestamp not in list(week_prefix[CC_to_consider].keys()):
                            ...
                            if CC_to_consider not in list(week_prefix_bogon.keys()):
                                week_prefix_bogon[CC_to_consider] = {}

                            if couple_timestamp_bogon not in list(week_prefix[CC_to_consider].keys()):
                                week_prefix[CC_to_consider][couple_timestamp_bogon] = []

                            if couple_timestamp_bogon not in list(week_prefix_bogon[CC_to_consider].keys()):
...
for CC in list(week_prefix.keys()):
    ...
    for couple_timestamp1 in list(week_prefix[CC].keys()):

        for elmt in week_prefix[CC][couple_timestamp1]:

            k = 0

            if couple_timestamp1 in list(week_prefix_bogon[CC].keys()):
...
for CC in list(week_prefix.keys()):
    ...
    for couple_timestamp11 in list(week_prefix[CC].keys()):

        if couple_timestamp11 in list(week_prefix_bogon[CC].keys()):

            with open(output_folder + 'LastMonth__number_unique_prefixes_at_IXP_' + CC + '.txt', 'a') as fh1:

                fh1.write('%s; %s; %s\n' % (couple_timestamp11, str(len(week_prefix[CC][couple_timestamp11])),
                                            str(len(week_prefix_bogon[CC][couple_timestamp11]))))
