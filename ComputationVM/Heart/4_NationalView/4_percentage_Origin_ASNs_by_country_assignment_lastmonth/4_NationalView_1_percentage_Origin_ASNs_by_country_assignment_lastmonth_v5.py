...
## Create a logfile:
name_log_file = 'Log_'+str(now_datetime) + '_' + 'National_View_1_Number_Origin_ASNs_visible_at_all_IXPs_in_a_country_lastmonth.txt'
...
for CC_key in IXP_OriginASes.keys():
        ...
        with open(filename, 'a') as fd:
            fd.write('%s;%s;%s;%s;%s;%s;%s\n'%('##Current_CC', 'Country name', 'Intersection OriginASNs & ASNs assigned to the country', 'num Origin ASNs seen at all IXPs in the country', 'Percentage of local ASNs compared to the ASNs seen at the IXP', 'num ASNs assigned by Afrinic to the CC', 'Percentage of ASNs assigned to the country seen at the IXP' ))
