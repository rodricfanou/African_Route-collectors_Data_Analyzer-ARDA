...
name_log_file = 'Log_' + str(
    now_datetime) + '_' + 'National_View_1_percentage_prefixes_by_country_assignment_lastmonth_better' + '.txt'
...
if len(List_all_tables) > 0:
    ...
    for ixp in list(IXP_collector.keys()):
        ...
        if CC_key not in list(week_prefix.keys()):
            week_prefix[CC_key] = []
...
for CC in list(week_prefix.keys()):

    with open(
            output_folder + 'List_prefixes_visibles_at_all_IXPs_of_a_country/List_of_prefixes_visibles_in_IXPs_of_' + CC + '.txt',
            'a') as fg:
...
for cc in list(CC_IXP.keys()):
    ...
    if cc in list(week_prefix.keys()):
                ...
                for prefix_assigned in list(set(CC_ASNs_AFRINIC[cc])):
                    if prefix_assigned not in intersection and prefix_assigned != '0.0.0.0/0':
                        ...
                        if cidrsOverlap(prefix_AF_adv, prefix_AF_assigned):
                            ...
                            print(
                                'Filling the file 1_percentage_of_allocated_prefixes_seen_at_all_IXPs_of_each_country.txt',
        ...
        print('percentage in % for Prefixes: ', ixp, len(list(set(week_prefix[cc]))), len(intersection),
              len(CC_ASNs_AFRINIC[cc]), percentage_found)

        with open(output_folder + '/1_percentage_of_allocated_prefixes_seen_at_all_IXPs_of_each_country.txt',
                  'a') as fd:
            fd.write('%s; %s; %s; %s; %s\n' % (
            ...
            with open(filename, 'a') as fd:
                if len(intersection) > 0:
                    try:
                        ...
                        fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                        current_CC, A.name, len(intersection), len(IXP_OriginASes[current_CC]), percentage_local_AF,
                        len(CC_ASNs_AFRINIC[current_CC]), percentage_local_AF2))

                    except:
                        fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                        current_CC, '', len(intersection), len(IXP_OriginASes[current_CC]), percentage_local_AF,
                        len(CC_ASNs_AFRINIC[current_CC]), percentage_local_AF2))
            ...
            for cc_external in list(CC_ASNs_AFRINIC.keys()):
                ...
                if current_CC != cc_external:
                    if cc_external not in list(ori_dict['external_AFRINIC'].keys()):
                        ...
                        if cc_external in list(IXP_OriginASes.keys()):
                            ...
                            with open(filename, 'a') as fd:
                                if len(intersection2) > 0:
                                    try:
                                        ...
                                        fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                                        cc_external, A.name, len(intersection2), len(IXP_OriginASes[current_CC]),
                                        percentage_external_AF, len(CC_ASNs_AFRINIC[cc_external]),
                                        percentage_external_AF2))

                                    except:
                                        fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                                        cc_external, '', len(intersection2), len(IXP_OriginASes[current_CC]),
                                        percentage_external_AF, len(CC_ASNs_AFRINIC[cc_external]),
                                        percentage_external_AF2))
            ...
            if cc_region not in list(ori_dict['RIPE'].keys()):
                ...
                with open(filename, 'a') as fd:
                    ...
                    try:
                        ...
                        fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                        cc_region, A.name, len(intersection3), len(IXP_OriginASes[current_CC]), percentage_RIPE,
                        len(CC_ASNs_RIPE[cc_region]), percentage_RIPE2))
                    except:
                        fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                        cc_region, '', len(intersection3), len(IXP_OriginASes[current_CC]), percentage_RIPE,
                        len(CC_ASNs_RIPE[cc_region]), percentage_RIPE2))
                ...
                with open(filename, 'a') as fd:
                    if len(intersection5) > 0:
                        try:
                            A = pycountry.countries.get(alpha2=cc_region)
                            fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                            cc_region, A.name, len(intersection5), len(IXP_OriginASes[current_CC]), percentage_ARIN,
                            len(CC_ASNs_ARIN[cc_region]), percentage_ARIN2))
                        except:
                            fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                            cc_region, '', len(intersection5), len(IXP_OriginASes[current_CC]), percentage_ARIN,
                            len(CC_ASNs_ARIN[cc_region]), percentage_ARIN2))
                ...
                with open(filename, 'a') as fd:
                    if len(intersection4) > 0:
                        try:
                            A = pycountry.countries.get(alpha2=cc_region)
                            fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                            cc_region, A.name, len(intersection4), len(IXP_OriginASes[current_CC]), percentage_APNIC,
                            len(CC_ASNs_APNIC[cc_region]), percentage_APNIC2))
                        except:
                            fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                            cc_region, '', len(intersection4), len(IXP_OriginASes[current_CC]), percentage_APNIC,
                            len(CC_ASNs_APNIC[cc_region]), percentage_APNIC2))
                ...
                with open(filename, 'a') as fd:
                    if len(intersection6) > 0:
                        try:
                            A = pycountry.countries.get(alpha2=cc_region)
                            fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                            cc_region, A.name, len(intersection6), len(IXP_OriginASes[current_CC]), percentage_LACNIC,
                            len(CC_ASNs_LACNIC[cc_region]), percentage_LACNIC2))
                        except:
                            fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                            cc_region, '', len(intersection6), len(IXP_OriginASes[current_CC]), percentage_LACNIC,
                            len(CC_ASNs_LACNIC[cc_region]), percentage_LACNIC2))
