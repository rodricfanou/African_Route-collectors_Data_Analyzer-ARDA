...
with open(output_folder + '/1_percentage_of_allocated_ASNs_seen_as_origin_ASNs_at_an_IXP.txt', 'a') as fd:
    fd.write('%s; %s; %s; %s; %s\n' % (
    '##IXP', 'Number of Origin ASNs found', 'Common ASNs to Origin ASNs and Assigned ASNs ', 'ASNs Assigned by Afrinic',
    'Percentage of Origin ASes at the IXP assigned by Afrinic'))
    fd.close()
...
for cc in CC_IXP:
    ...
    for ixp in CC_IXP[cc]:
        ...
        if ixp in list(week_ASN.keys()):
            ...
            print('percentage in % for Origin ASes:', ixp, len(list(set(week_ASN[ixp]))), len(intersection),
...
for ixp in list(IXP_OriginASes.keys()):
    ...
    with open(filename, 'a') as fd:
        ...
        'num Origin ASNs seen at the IXP', 'Percentage of local ASNs compared to the ASNs seen at the IXP',
        'num ASNs assigned by Afrinic to the CC', 'Percentage of ASNs assigned to the country seen at the IXP'))
                ...
                for cc_external in list(CC_ASNs_AFRINIC.keys()):
                    ...
                    if current_CC != cc_external:
                        if cc_external not in list(ori_dict['external_AFRINIC'].keys()):
                            ...
                            try:
                                percentage_external_AF2 = float(len(intersection2)) / float(
                                    len(CC_ASNs_AFRINIC[cc_external])) * 100
                            except:
                ...
                for cc_region in CC_ASNs_APNIC:
                    if current_CC != cc_region:
                        if cc_region not in list(ori_dict['APNIC'].keys()):
                            ...
                            try:
                                percentage_APNIC2 = float(len(intersection4)) / float(
                                    len(CC_ASNs_APNIC[cc_region])) * 100
                            except:
                ...
                for cc_region in CC_ASNs_LACNIC:
                    if current_CC != cc_region:
                        if cc_region not in list(ori_dict['LACNIC'].keys()):
                            ...
                            try:
                                percentage_LACNIC2 = float(len(intersection6)) / float(
                                    len(CC_ASNs_LACNIC[cc_region])) * 100
                            except:
