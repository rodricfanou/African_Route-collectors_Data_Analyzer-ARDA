...
if List_all_tables > 0:
    ...
    for ixp in list(IXP_collector.keys()):
        ...
        for window in couples_year_month:
            ...
            query = "select distinct Network, IP_version, OriginAS, ASpath from Data__" + str(
                int(window[0])) + "_" + str(int(window[1])) + " where Timestamp >= %s and Timestamp <= %s  and ("
            ...
            for couple_timestamp in List_beg_end_each_week:
                ...
                if len(data) > 0:
                    ...
                    while (i < len(data)):
                        ...
                        create_output1 = open(
                            output_folder + 'LastMonth_' + ixp + '_List_ASNs_announcing_' + IPversion + '.txt', 'a')
                        ...
                        if OriginAS not in week_ASN[ixp][week][IPversion]:
                            ...
                            create_output1.write(
                                str(week) + '; ' + '; '.join(tab11) + '; ' + '; ' + str(OriginAS) + '\n')
    ...
    for ixp in list(week_ASN.keys()):
        ...
        for week in list(week_ASN[ixp].keys()):
            ...
            for IPversion in list(week_ASN[ixp][week].keys()):
                create_output = open(
                    output_folder + 'LastMonth_' + ixp + '_Number_ASNs_announcing_' + IPversion + '.txt', 'a')

                create_output.write(str(week) + '; ' + '; '.join(tab) + '; ' + str(
                    len(list(set(week_ASN[ixp][week][IPversion])))) + '\n')
