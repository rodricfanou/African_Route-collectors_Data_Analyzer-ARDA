...
## Create a logfile:
name_log_file = 'Log_'+str(now_datetime) + '_' + '1_percentage_prefixes_by_country_assignment_lastmonth_better' + '.txt'
	...
	if len(data)>0:
   		 while (i<len(data)):
       			...
       			if '.' in row[0] or ':' in row[0]:
          			...
          			CC = row[2]
	  			if  CC != '':
            				if CC not in CC_ASNs_RIPE.keys():
            					CC_ASNs_RIPE[CC] = []
        ...
        if len(data)>0:
                 while (i<len(data)):
                        ...
                        if '.' in row[0] or ':' in row[0]:
                                ...
                                CC = row[2]
                                if  CC != '':
                                        if CC not in CC_ASNs_ARIN.keys():
                                                CC_ASNs_ARIN[CC] = []
        ...
        if len(data)>0:
                 while (i<len(data)):
                        ...
                        if '.' in row[0] or ':' in row[0]:
                                ...
                                CC = row[2]
                                if  CC != '':
                                        if CC not in CC_ASNs_APNIC.keys():
                                                CC_ASNs_APNIC[CC] = []
        ...
        if len(data)>0:
                 while (i<len(data)):
                        ...
                        if '.' in row[0] or ':' in row[0]:
                                ...
                                CC = row[2]
                                if  CC != '':
                                        if CC not in CC_ASNs_LACNIC.keys():
                                                CC_ASNs_LACNIC[CC] = []
...
for cc in CC_IXP:
    ...
    for ixp in CC_IXP[cc]:
        ...
        if ixp in week_prefix.keys():
                ...
                if prefix_adv != '0.0.0.0/0':
                    ...
                    for prefix_assigned in CC_ASNs_AFRINIC[cc]:
                            ...
                            if cidrsOverlap(prefix_AF_adv, prefix_AF_assigned):
                                ...
                                print 'Filling the file 1_percentage_of_allocated_prefixes_seen_at_an_IXP.txt', prefix_AF_adv, ' overlaps ', prefix_AF_assigned
            ...
            print 'percentage in % for Prefixes: ', ixp, len(list(set(week_prefix[ixp]))), len(intersection), len(CC_ASNs_AFRINIC[cc]), percentage_found
...
for ixp in IXP_OriginASes.keys():    
    ...
    with open(filename, 'a') as fd:
        fd.write('%s;%s;%s;%s;%s;%s;%s;%s\n'%('##Current_CC', 'Country name', 'Intersection visible prefixes & prefixes assigned to the country', 'num prefixes seen at the IXP', 'Percentage_local_AF', 'num prefixes assigned by Afrinic to the CC', 'Percentage_local_AF2', 'RIR' ))
    ...
    for CC in CC_IXP.keys():
		...
		if ixp not in Listrest_ASes.keys():
			Listrest_ASes[ixp] = []
                ...
                with open(filename, 'a') as fd:
                    if len(intersection) >0:
                        try:
                            ...
                            fd.write('%s;%s;%s;%s;%s;%s;%s;%s\n'%(current_CC, A.name, len(intersection), len(IXP_OriginASes[ixp]), percentage_local_AF, len( CC_ASNs_AFRINIC[current_CC] ), percentage_local_AF2, 'AFRINIC'))
                        except:
                            fd.write('%s;%s;%s;%s;%s;%s;%s;%s\n'%(current_CC, '', len(intersection), len(IXP_OriginASes[ixp]), percentage_local_AF, len( CC_ASNs_AFRINIC[current_CC] ), percentage_local_AF2, 'AFRINIC' ))
			    ...
			    intersection22 = copy.deepcopy(intersection2)
		            ...
		            #    if prefix_adv1 in Listrest_ASes[ixp]:
						...
						if prefix_adv in Listrest_ASes[ixp]:
                            ...
                            try:
                                percentage_external_AF = 100*(float(len(intersection22))/float(len( IXP_OriginASes[ixp])))
                            ...
                            with open(filename, 'a') as fd:
                                if len(intersection2) > 0:
                                    try:
                                        ...
                                        fd.write('%s;%s;%s;%s;%s;%s;%s;%s\n'%(cc_external, A.name , len(intersection2), len(IXP_OriginASes[ixp]), percentage_external_AF, len(CC_ASNs_AFRINIC[cc_external]),  percentage_external_AF2, 'AFRINIC'))
                                    except:
                                        fd.write('%s;%s;%s;%s;%s;%s;%s;%s\n'%(cc_external, '' , len(intersection2), len(IXP_OriginASes[ixp]), percentage_external_AF, len(CC_ASNs_AFRINIC[cc_external]),  percentage_external_AF2, 'AFRINIC'))
                                ...
                                with open(filename, 'a') as fd:
                                    if len(intersection3)> 0:
                                    	try:
                                        	...
                                        	fd.write('%s;%s;%s;%s;%s;%s;%s;%s\n'%(cc_region, A.name, len(intersection3), len(IXP_OriginASes[ixp]), percentage_RIPE, len(CC_ASNs_RIPE[cc_region]),  percentage_RIPE2, 'RIPE'))

                                    	except:
                                        	fd.write('%s;%s;%s;%s;%s;%s;%s;%s\n'%(cc_region, '', len(intersection3), len(IXP_OriginASes[ixp]), percentage_RIPE, len(CC_ASNs_RIPE[cc_region]), percentage_RIPE2, 'RIPE'))
						...
						if prefix_adv in Listrest_ASes[ixp]:
                            ...
                            with open(filename, 'a') as fd:
                                if len(intersection5) > 0:
                                    try:
                                        A = pycountry.countries.get(alpha2=cc_region)
                                        fd.write('%s;%s;%s;%s;%s;%s;%s;%s\n'%(cc_region, A.name, len(intersection5), len(IXP_OriginASes[ixp]), percentage_ARIN, len( CC_ASNs_ARIN[cc_region]), percentage_ARIN2, 'ARIN'))
                                    except:
                                        fd.write('%s;%s;%s;%s;%s;%s;%s;%s\n'%(cc_region, '', len(intersection5) , len(IXP_OriginASes[ixp]) , percentage_ARIN, len( CC_ASNs_ARIN[cc_region]), percentage_ARIN2, 'ARIN'))
			    ...
			    if len(intersection4) > 0:
                            ...
                            with open(filename, 'a') as fd:
                                if len(intersection4)>0:
                                    try:
                                        A = pycountry.countries.get(alpha2=cc_region)
                                        fd.write('%s;%s;%s;%s;%s;%s;%s;%s\n'%(cc_region, A.name, len(intersection4), len(IXP_OriginASes[ixp]), percentage_APNIC, len( CC_ASNs_APNIC[cc_region] ),  percentage_APNIC2, 'APNIC'))
                                    except:
                                        fd.write('%s;%s;%s;%s;%s;%s;%s;%s\n'%(cc_region,'', len(intersection4), len(IXP_OriginASes[ixp]) , percentage_APNIC, len(CC_ASNs_APNIC[cc_region] ), percentage_APNIC2, 'APNIC'))
			    ...
			    if len(intersection6) > 0:
                            ...
                            with open(filename, 'a') as fd:
                                if len(intersection6)>0:
                                    try:
                                        A = pycountry.countries.get(alpha2=cc_region)
                                        fd.write('%s;%s;%s;%s;%s;%s;%s;%s\n'%(cc_region, A.name, len(intersection6), len(IXP_OriginASes[ixp]), percentage_LACNIC, len( CC_ASNs_LACNIC[cc_region] ), percentage_LACNIC2, 'LACNIC'))
                                    except:
                                        fd.write('%s;%s;%s;%s;%s;%s;%s;%s\n'%(cc_region,'', len(intersection6), len(IXP_OriginASes[ixp]), percentage_LACNIC, len( CC_ASNs_LACNIC[cc_region] ), percentage_LACNIC2, 'LACNIC'))
