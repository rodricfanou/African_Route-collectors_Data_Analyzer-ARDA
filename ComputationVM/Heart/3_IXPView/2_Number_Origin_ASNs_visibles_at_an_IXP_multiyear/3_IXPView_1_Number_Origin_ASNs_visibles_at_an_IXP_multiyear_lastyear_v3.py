...
if List_all_tables >0:
    ...
    for ixp in IXP_collector.keys():
        ...
        create_output_lastyear.write('###Month-Year; Visible Origin ASNs \n')
        
        create_output_lastyear_2bytesASN = open(output_folder+'LastYear__2bytes_list_visible_ASNs_at_IXP_'+ixp+'.txt', 'a')
        
        create_output_lastyear_2bytesASN.write('###Month-Year; Visible 2bytes Origin ASNs \n')
        
        create_output_lastyear_4bytesASN = open(output_folder+'LastYear__4bytes_list_visible_ASNs_at_IXP_'+ixp+'.txt', 'a')
        
        create_output_lastyear_4bytesASN.write('###Month-Year; Visible 4bytes Origin ASNs \n')
        ...
        create_output_MultiYear_2bytesASN = open(output_folder+'MultiYear__2bytes_list_visible_ASNs_at_IXP_'+ixp+'.txt', 'a')
        
        create_output_MultiYear_2bytesASN.write('###Month-Year; Visible 2bytes Origin ASNs \n')
            
        create_output_MultiYear_4bytesASN = open(output_folder+'MultiYear__4bytes_list_visible_ASNs_at_IXP_'+ixp+'.txt', 'a')
        
        create_output_MultiYear_4bytesASN.write('###Month-Year; Visible 4bytes Origin ASNs \n')
        ...
        if ixp not in year_ASN_2bytes.keys():
            year_ASN_2bytes[ixp] = {}
        
        if ixp not in year_ASN_4bytes.keys():
            year_ASN_4bytes[ixp] = {}
        ...
        if ixp not in month_ASN_2bytes.keys():
            month_ASN_2bytes[ixp] = {}

        if ixp not in month_ASN_4bytes.keys():
        ...
        for year in yearList:
            ...
            if year not in year_ASN_2bytes[ixp].keys():
                year_ASN_2bytes[ixp][year] = []

            if year not in year_ASN_4bytes[ixp].keys():
                        ...
                        while (i<len(data)):
                            ...
                            for OriginASNs_elmt in OriginASNs :
                                ...
                                if OriginASNs_elmt not in year_ASN[ixp][year]:
                                    ...
                                    if OriginASNs_elmt >= 0 and OriginASNs_elmt <= 65535 and OriginASNs_elmt not in year_ASN_2bytes[ixp][year]:
                                        year_ASN_2bytes[ixp][year].append(OriginASNs_elmt)
                                        create_output_MultiYear_2bytesASN.write(str(year) + '; ' + str(OriginASNs_elmt) + '\n')
                                    ...
                                    if OriginASNs_elmt >= 65536 and OriginASNs_elmt <= 4294967295 and OriginASNs_elmt not in year_ASN_4bytes[ixp][year]:
                                        year_ASN_4bytes[ixp][year].append(OriginASNs_elmt)
                                        create_output_MultiYear_4bytesASN.write(str(year) + '; ' + str(OriginASNs_elmt) + '\n')
                                ...
                                if (year, month) in lastYearList:
                                    ...
                                    if OriginASNs_elmt not in month_ASN[ixp][str(month)+'-'+str(year)]:
                                        ...
                                        if str(month)+'-'+str(year) not in month_ASN_2bytes[ixp].keys():
                                            month_ASN_2bytes[ixp][str(month)+'-'+str(year)] = []
                                        
                                        if str(month)+'-'+str(year) not in month_ASN_4bytes[ixp].keys():
                                        ...
                                        #2 bytes : AS 0 to 65535  ## Reserved: 64512 to 65534 for private; few others such as 0 and 65535
                                        if OriginASNs_elmt >= 0 and OriginASNs_elmt <= 65535 and OriginASNs_elmt not in month_ASN_2bytes[ixp][str(month)+'-'+str(year)]:
                                            month_ASN_2bytes[ixp][str(month)+'-'+str(year)].append(OriginASNs_elmt)
                                            create_output_lastyear_2bytesASN.write(str(str(month)+'-'+str(year)) + '; ' + str(OriginASNs_elmt) + '\n')

    
                                        # 4 bytes : AS 65,536 - 4294,967,295; The first thing to notice about these numbers is that they include all of the older 2-byte ASNs, 0 through 65535
                                        # A 4-byte ASN between 0 and 65535 is called a mappable ASN
                                        if OriginASNs_elmt >= 65536 and OriginASNs_elmt <= 4294967295 and OriginASNs_elmt not in month_ASN_4bytes[ixp][str(month)+'-'+str(year)]:
                                            month_ASN_4bytes[ixp][str(month)+'-'+str(year)].append(OriginASNs_elmt)
                                            create_output_lastyear_4bytesASN.write(str(str(month)+'-'+str(year)) + '; '+ str(OriginASNs_elmt) + '\n')
        ...
        if ixp in month_ASN.keys():
            ...
            create_output_lastyear = open(output_folder+'LastYear__number_visible_ASNs_at_IXP_'+ixp+'.txt', 'a')
            
            create_output_lastyear.write('##Month-Year; Number of Origin ASNs; Number of 2bytes Origin ASNs; Number of 4bytes Origin ASNs \n')
            ...
            for elmt in month_ASN[ixp].keys():
                ...
                create_output_lastyear.write(elmt+ '; '+ str(len(month_ASN[ixp][elmt])) + '; ' +  str(Two_Bytes_ASNs) + '; ' + str(Four_Bytes_ASNs) + '\n')
        ...
        if ixp in year_ASN.keys():
            ...
            create_output_MultiYear =  open(output_folder+'MultiYear__number_visible_ASNs_at_IXP_'+ixp+'.txt', 'a')
            
            create_output_MultiYear.write('##Year' + '; Number of OriginASNs ; Number of 2bytes Origin ASNs; Number of 4bytes Origin ASNs \n')
            ...
            #for year in year_prefix[ixp].keys():
                create_output_MultiYear.write(str(year)+ '; '+ str(len(year_ASN[ixp][year]))  + '; ' +  str(Two_Bytes_ASNs) + '; ' + str(Four_Bytes_ASNs) + '\n')
