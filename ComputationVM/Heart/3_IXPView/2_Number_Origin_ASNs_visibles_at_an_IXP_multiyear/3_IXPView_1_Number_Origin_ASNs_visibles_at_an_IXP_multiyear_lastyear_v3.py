...
if List_all_tables > 0:
    ...
    for ixp in list(IXP_collector.keys()):
        ...
        create_output_lastyear_2bytesASN = open(
            output_folder + 'LastYear__2bytes_list_visible_ASNs_at_IXP_' + ixp + '.txt', 'a')

        create_output_lastyear_2bytesASN.write('###Month-Year; Visible 2bytes Origin ASNs \n')

        create_output_lastyear_4bytesASN = open(
            output_folder + 'LastYear__4bytes_list_visible_ASNs_at_IXP_' + ixp + '.txt', 'a')
        ...
        create_output_MultiYear_2bytesASN = open(
            output_folder + 'MultiYear__2bytes_list_visible_ASNs_at_IXP_' + ixp + '.txt', 'a')

        create_output_MultiYear_2bytesASN.write('###Month-Year; Visible 2bytes Origin ASNs \n')

        create_output_MultiYear_4bytesASN = open(
            output_folder + 'MultiYear__4bytes_list_visible_ASNs_at_IXP_' + ixp + '.txt', 'a')
                                    ...
                                    if OriginASNs_elmt not in month_ASN[ixp][str(month) + '-' + str(year)]:
                                        ...
                                        if OriginASNs_elmt >= 0 and OriginASNs_elmt <= 65535 and OriginASNs_elmt not in \
                                            ...
                                            create_output_lastyear_2bytesASN.write(
                                                str(str(month) + '-' + str(year)) + '; ' + str(OriginASNs_elmt) + '\n')
        ...
        if ixp in list(month_ASN.keys()):
            ...
            create_output_lastyear.write(
                '##Month-Year; Number of Origin ASNs; Number of 2bytes Origin ASNs; Number of 4bytes Origin ASNs \n')
        ...
        if ixp in list(year_ASN.keys()):
            ...
            create_output_MultiYear.write(
                '##Year' + '; Number of OriginASNs ; Number of 2bytes Origin ASNs; Number of 4bytes Origin ASNs \n')
