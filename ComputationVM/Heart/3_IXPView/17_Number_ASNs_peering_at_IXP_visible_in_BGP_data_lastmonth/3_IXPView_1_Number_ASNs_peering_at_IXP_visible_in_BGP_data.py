...
name_log_file = 'Log_' + str(
    now_datetime) + '_' + '17_Number_ASNs_peering_at_IXP_visible_in_BGP_data_lastmonth' + '.txt'
...
if List_all_tables > 0:
    ...
    for ixp in list(IXP_collector.keys()):
        ...
        create_output_2bytesASN = open(
            output_folder + 'LastMonth__2bytes_list_visible_ASNs_peering_at_IXP_' + ixp + '.txt', 'a')

        create_output_2bytesASN.write(
            '###Num Week; Timestamp beginning;  Timestamp end;  Datetime  beginning;  Datetime end; Visible 2bytes ASNs peering at the IXP \n')

        create_output_4bytesASN = open(
            output_folder + 'LastMonth__4bytes_list_visible_ASNs_peering_at_IXP_' + ixp + '.txt', 'a')

        create_output_4bytesASN.write(
            '###Num Week; Timestamp beginning;  Timestamp end;  Datetime  beginning;  Datetime end; Visible 4bytes ASNs peering at the IXP \n')
        ...
        for window in couples_year_month:
            ...
            query = "select distinct ASPath from Data__" + str(int(window[0])) + "_" + str(
    ...
    for ixp in list(week_ASN.keys()):
        ...
        create_output.write(
            '##Num Week' + '; Timestampbeg; Timestampend; Number of OriginAS; Number of 2bytes OriginAS; Number of 4bytes ASN peering at the IXP \n')
