...
output_folder = '../../Computation_outputs/2_Number_Origin_ASNs_visibles_at_an_IXP_lastmonth/'
...
if List_all_tables > 0:
    ...
    if int(tab2[0]) == int(tab1[0]):
        ...
        while num < week_number_last_day + 1:
            ...
            List_beg_end_each_week.append(
                str(begweek) + '__' + str(endweek) + '__' + str(begweek1) + '  00:00:00__' + str(
                    endweek1) + '  00:00:00')
        ...
        while num > 0:
            ...
            List_beg_end_each_week.insert(0, (
                        str(begweek) + '__' + str(endweek) + '__' + str(begweek1) + '  00:00:00__' + str(
                    endweek1) + '  00:00:00'))
            # print begweek, endweek
            num -= 1
        ...
        while int(endweek) != keep_tsp:
            ...
            List_beg_end_each_week.insert(ind, (
                        str(begweek) + '__' + str(endweek) + '__' + str(begweek1) + '  00:00:00__' + str(
                    endweek1) + '  00:00:00'))
            # print begweek, endweek
            ind += 1
    ...
    for ixp in list(IXP_collector.keys()):
        ...
        create_output = open(output_folder + 'LastMonth__list_visible_ASNs_at_IXP_' + ixp + '.txt', 'a')

        create_output.write(
            '###Num Week; Timestamp beginning;  Timestamp end;  Datetime  beginning;  Datetime end; Visible Origin ASNs \n')

        create_output_2bytesASN = open(output_folder + 'LastMonth__2bytes_list_visible_ASNs_at_IXP_' + ixp + '.txt',
                                       'a')

        create_output_2bytesASN.write(
            '###Num Week; Timestamp beginning;  Timestamp end;  Datetime  beginning;  Datetime end; Visible 2bytes Origin ASNs \n')

        create_output_4bytesASN = open(output_folder + 'LastMonth__4bytes_list_visible_ASNs_at_IXP_' + ixp + '.txt',
                                       'a')

        create_output_4bytesASN.write(
                ...
                if len(data) > 0:
                    while (i < len(data)):
                        ...
                        if '{' not in row[0] and ',' not in row[0] and '}' not in row[0]:
                            ...
                            try:
                                if int(row[0]) not in OriginASNs:
                            ...
                            path = row[1].split(' ')
                            try:
                                if int(str(path[-2]).strip()) not in OriginASNs:
                        ...
                        for OriginASNs_elmt in OriginASNs:
                            ...
                            if OriginASNs_elmt not in week_ASN[ixp][week]:
                                ...
                                create_output.write(
                                    str(week) + '; ' + '; '.join(tab) + '; ' + str(OriginASNs_elmt) + '\n')
    ...
    for ixp in list(week_ASN.keys()):
        ...
        create_output = open(output_folder + 'LastMonth__number_visible_ASNs_at_IXP_' + ixp + '.txt', 'a')

        create_output.write(
            '##Num Week' + '; Timestampbeg; Timestampend; Number of OriginAS; Number of Visible 2bytes Origin ASN; Number of Visible 4bytes Origin ASN  \n')
        ...
        for week in list(week_ASN[ixp].keys()):
            ...
            create_output.write(
                str(week) + '; ' + '; '.join(tab) + '; ' + str(len(list(set(week_ASN[ixp][week])))) + '; ' + str(
                    Two_Bytes_ASNs) + '; ' + str(Four_Bytes_ASNs) + '\n')
