...
with open(filename, 'a') as fd:
    ...
    'CC', 'IXP', 'month-Year', 'Number of Origin ASNs assigned to the country visibles at the IXP',
...
with open(filename, 'a') as fd:
    fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
    'CC', 'IXP', 'Year', 'Number of Origin ASNs assigned to the country visibles at the IXP',
    'Number of Origin ASNs visibles at the IXP', 'Percentage of local Origin ASNs',
    'Percentage of External Origin ASNs'))
                    ...
                    if ixp == ixp1:
                        ...
                        with open(filename, 'a') as fd:
                            ...
                            try:
                                ...
                                fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                                current_CC, A, elmt, len(intersection), len(month_ASN[ixp][elmt]), percentage_local_AF,
                                100 - percentage_local_AF))
                            except:
                                fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                                current_CC, '', elmt, len(intersection), len(month_ASN[ixp][elmt]), percentage_local_AF,
                                100 - percentage_local_AF))
                    ...
                    if ixp == ixp1:
                        ...
                        with open(filename, 'a') as fd:
                            ...
                            try:
                                A = pycountry.countries.get(alpha2=current_CC)
                                fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                                current_CC, A, year, len(intersection), len(year_ASN[ixp][year]), percentage_local_AF,
                                100 - percentage_local_AF))
                            except:
                                fd.write('%s;%s;%s;%s;%s;%s;%s\n' % (
                                current_CC, '', year, len(intersection), len(year_ASN[ixp][year]), percentage_local_AF,
                                100 - percentage_local_AF))
