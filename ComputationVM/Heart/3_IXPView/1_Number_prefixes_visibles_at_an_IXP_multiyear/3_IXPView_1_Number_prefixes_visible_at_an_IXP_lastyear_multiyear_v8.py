...
if List_all_tables > 0:
    ...
    for ixp in list(IXP_collector.keys()):
        ...
        if ixp not in list(year_prefix_slash.keys()):
            year_prefix_slash[ixp] = {}

        if ixp not in list(month_prefix_slash.keys()):
        ...
        for year in yearList:
            ...
            if year not in list(year_prefix_bogon[ixp].keys()):
                year_prefix_bogon[ixp][year] = []

            if year not in list(year_prefix_slash[ixp].keys()):
                        ...
                        while (i < len(data)):
                            ...
                            if 'None' not in prefix:
                                ...
                                try:
                                    ...
                                    if str(tableau_net_pref[-1]) not in list(year_prefix_slash[ixp][year].keys()):
                                        year_prefix_slash[ixp][year][str(tableau_net_pref[-1])] = []
                                    ...
                                    create_output_slash = open(
                                        output_folder + 'Multiyear__list_visible_prefixes_at_IXP_' + ixp + '_with_slash_' + str(
                                            tableau_net_pref[-1]) + '.txt', 'a')
                                    create_output_slash.write(str(year) + '; ' + str(prefix) + '\n')
                                    create_output_slash.close()
                                    ...
                                    if (year, month) in lastYearList:
                                        ...
                                        if str(month) + '-' + str(year) not in list(month_prefix_bogon[ixp].keys()):
                                            month_prefix_bogon[ixp][str(month) + '-' + str(year)] = []

                                        if str(month) + '-' + str(year) not in list(month_prefix_slash[ixp].keys()):
                                        ...
                                        if prefix in year_prefix_bogon[ixp][year]:
                                            ...
                                            if prefix not in month_prefix_bogon[ixp][str(month) + '-' + str(year)]:
                                                ...
                                                create_output_lastyear.write(
                                                    str(month) + '-' + str(year) + '; ' + str(prefix) + '; bogon\n')
                                        ...
                                        if str(tableau_net_pref[-1]) not in list(
                                                month_prefix_slash[ixp][str(month) + '-' + str(year)].keys()):
                                            month_prefix_slash[ixp][str(month) + '-' + str(year)][
                                                str(tableau_net_pref[-1])] = []

                                        if prefix not in month_prefix_slash[ixp][str(month) + '-' + str(year)][
                                            str(tableau_net_pref[-1])]:
                                            month_prefix_slash[ixp][str(month) + '-' + str(year)][
                                                str(tableau_net_pref[-1])].append(prefix)

                                        with open(
                                                output_folder + 'Lastyear__list_visible_prefixes_at_IXP_' + ixp + '_with_slash_' + str(
                                                        tableau_net_pref[-1]) + '.txt', 'a') as fg:
                                            fg.write('%s; %s \n' % (str(month) + '-' + str(year), str(prefix)))
        ...
        if ixp in list(month_prefix.keys()):
            ...
            create_output_lastyear = open(output_folder + 'LastYear__number_visible_prefixes_at_IXP_' + ixp + '.txt',
                                          'a')

            create_output_lastyear.write(
                '###Month-Year; number visible prefixes at IXP; number of visible bogon prefixes at IXP \n')

            for elmt in list(month_prefix[ixp].keys()):
                create_output_lastyear.write(elmt + '; ' + str(len(month_prefix[ixp][elmt])) + '; ' + str(
                    len(month_prefix_bogon[ixp][elmt])) + '\n')

            print()
            print((month_prefix_slash[ixp]))
            print()

            create_output_lastyear_slash = open(
                output_folder + 'LastYear__number_visible_prefixes_at_IXP_with_slash_' + ixp + '.txt', 'a')

            create_output_lastyear_slash.write(
                '###Month-Year; number visible prefixes at IXP of length prefix length \n')

            if len(list(month_prefix_slash[ixp].keys())) > 0:

                for year in list(month_prefix_slash[ixp].keys()):

                    if len(month_prefix_slash[ixp][year]) > 0:

                        for slash_prefix in list(month_prefix_slash[ixp][year].keys()):
                            create_output_lastyear_slash.write(str(year) + '; ' + str(slash_prefix) + '; ' + str(
                                len(month_prefix_slash[ixp][year][slash_prefix])) + '\n')
        ...
        if ixp in list(year_prefix.keys()):
            ...
            create_output_MultiYear = open(output_folder + 'MultiYear__number_visible_prefixes_at_IXP_' + ixp + '.txt',
                                           'a')

            create_output_MultiYear.write(
                '###Year;  number visible prefixes at IXP; number of visible bogon prefixes at IXP \n')

            for year in yearList:
                create_output_MultiYear.write(str(year) + '; ' + str(len(year_prefix[ixp][year])) + '; ' + str(
                    len(year_prefix_bogon[ixp][year])) + '\n')

            # print
            # print year_prefix_slash[ixp]
            # print

            create_output_Multiyear_slash = open(
                output_folder + 'MultiYear__number_visible_prefixes_at_IXP_with_slash_' + ixp + '.txt', 'a')

            if len(list(year_prefix_slash[ixp].keys())) > 0:

                for year in list(year_prefix_slash[ixp].keys()):

                    if len(year_prefix_slash[ixp][year]) > 0:

                        for slash_prefix in list(year_prefix_slash[ixp][year].keys()):
                            create_output_Multiyear_slash.write(str(year) + '; ' + str(slash_prefix) + '; ' + str(
                                len(year_prefix_slash[ixp][year][slash_prefix])) + '\n')
