...
name_log_file = 'Log_' + str(now_datetime) + '_' + '7_1_percentage_IPv4_IPv6_blocks_country_level' + '.txt'
...
if List_all_tables > 0:
    ...
    for window in couples_year_month:
        ...
        if "Data__" + str(int(window[0])) + "_" + str(int(window[1])) in List_all_tables:
            ...
            query = "select distinct Data__" + str(int(window[0])) + "_" + str(
                int(window[1])) + ".Network, Data__" + str(int(window[0])) + "_" + str(
                int(window[1])) + ".RouteCollector, Data__" + str(int(window[0])) + "_" + str(
                int(window[1])) + ".IP_version, AllRouteCollectors.CC from Data__" + str(int(window[0])) + "_" + str(
                int(window[1])) + ", AllRouteCollectors where AllRouteCollectors.RouteCollector = Data__" + str(
                int(window[0])) + "_" + str(
                int(window[1])) + ".RouteCollector and Timestamp >= %s and Timestamp <= %s and Data__" + str(
                int(window[0])) + "_" + str(int(window[1])) + ".IP_version <> 'None' ;"
                    ...
                    for ixp in list(IXP_collector.keys()):  # delete de 2 for the complete continent
                        ...
                        for route_collector in IXP_collector[ixp]:  # delete de 2 for the complete continent
                            ...
                            if str(route_collector) == str(route_collector_extracted):
                                ...
                                print('RouteCollector ', route_collector,
                                      'does not appear in  table Data__' + str(int(window[0])) + "_" + str(
                                          int(window[1])))
    ...
    command = "mkdir " + output_folder + "Percentage_IPv6_assigned_to_country_appearing_per_IXP/"
    os.system(command)
    ...
    for IPversion in list(Intersection.keys()):
        ...
        with open(filename, 'a') as fg:
            ...
            'IPversion', 'ixp', 'CC_AF', 'percentage_IPv4_blocks_appearing_at_IXP',
            'len(Intersection_IPv4_blocks_appearing)', 'len(IPv4blocks_assigned_to_country)'))
        ...
        for ixp in list(Intersection[IPversion].keys()):
            ...
            for CC_AF in list(Intersection[IPversion][ixp].keys()):
                ...
                with open(filename, 'a') as fg:
                    ...
                    try:
                        perc_IP_blocks = 100 * (float(len(list(set(Intersection[IPversion][ixp][CC_AF]))))) / (
                            float(len(list(set(CC_prefixes_AFRINIC[IPversion][CC_AF])))))
                    except:
                        perc_IP_blocks = 0

                    fg.write('%s; %s; %s; %s; %s; %s\n' % (
                    IPversion, ixp, CC_AF, perc_IP_blocks, len(list(set(Intersection[IPversion][ixp][CC_AF]))),
                    len(list(set(CC_prefixes_AFRINIC[IPversion][CC_AF])))))

                with open(filename1, 'a') as fgh:
                    for line in list(set(Intersection[IPversion][ixp][CC_AF])):
                        line = str(line).strip()
                        fgh.write('%s\n' % (line))
