        ...
        for ixp in Intersection[IPversion].keys():
            ...
            for CC_AF in Intersection[IPversion][ixp].keys():
                ...
                with open (filename, 'a') as fg:
                    ...
                    try:
                        perc_IP_blocks = 100 * ( float(len( list(set(Intersection[IPversion][ixp][CC_AF])) ))) / (float(len( list(set( CC_prefixes_AFRINIC[IPversion][CC_AF]))  )))
                    except:
                        perc_IP_blocks = 0
            
                    fg.write ('%s; %s; %s; %s; %s; %s\n' %(IPversion, ixp, CC_AF,  perc_IP_blocks, len(list(set(Intersection[IPversion][ixp][CC_AF]))), len(list(set(CC_prefixes_AFRINIC[IPversion][CC_AF])))))
