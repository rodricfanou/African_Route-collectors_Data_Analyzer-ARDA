...
for ixp in list(month_ASN.keys()):

    for month_year_current in list(month_ASN[ixp].keys()):

        tab = month_year_current.split('-')

        IXP_OriginASes = []

        IXP_OriginASes = copy.deepcopy(month_ASN[ixp][month_year_current])
                    ...
                    for cc_external in list(CC_ASNs_AFRINIC.keys()):
                        ...
                        if current_CC != cc_external:
                            if cc_external not in list(ori_dict['external_AFRINIC'].keys()):
                                ...
                                try:
                                    percentage_external_AF = (100 * (float(len(intersection2)))) / float(
                                        len(Listrest_ASes))
                                except:
