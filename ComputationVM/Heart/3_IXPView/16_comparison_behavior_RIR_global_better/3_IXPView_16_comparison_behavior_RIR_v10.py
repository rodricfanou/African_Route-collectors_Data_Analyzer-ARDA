...
List_RIRs_prefixes = ['IPv4_ressources_AFRINIC', 'IPv4_ressources_ARIN', 'IPv4_ressources_APNIC',
        ...
        for window in couples_year_month:
            ...
            for couple_timestamp in List_beg_end_each_week:
                ...
                try:
                    ...
                    print('Alert table ', "Data__" + str(lastMonthList[0][0]) + "_" + str(lastMonthList[0][1]),
                          ' does not exist yet')
                                    ...
                                    if prefix not in week_prefix[ixp]:
                                        ...
                                        while j < len(Keep_all_prefixes):
                                            ...
                                            if cidrsOverlap(prefix_IX, prefix_RIR):
                                                ...
                                                with open(
                                                        output_folder + 'Output_Intersection_week_prefix___' + ixp + '.txt',
                                                        'a') as fgg:
                                                    fgg.write('%s\n' % (prefix))

                                                    print('We added prefix ', prefix, ' to ',
                                                          output_folder + 'Output_Intersection_week_prefix___' + ixp + '.txt')
...
for IXP_in_world in list(Prefixes_on_the_internet.keys()):
    ...
    for Global_prefix in Prefixes_on_the_internet[IXP_in_world]:
        ...
        for ixp in list(week_prefix.keys()):
            ...
            create_file1 = open(
                output_folder + 'List_prefix_announcements_IXPShorterThanUpstream__for__' + ixp + '.txt', 'a')
            ...
            create_file2 = open(output_folder + 'List_prefix_announcements_IXPLongerThanUpstream__for__' + ixp + '.txt',
                                'a')
            ...
            create_file3 = open(output_folder + 'List_prefix_announcements_IXPBalancedUpstream__for__' + ixp + '.txt',
                                'a')
                        ...
                        if cidrsOverlap(prefix_seen_IX, prefix_Internet):
                            ...
                            if int(tab_IXP[-1]) > int(tab_Internet[-1]):
                                ...
                                print(int(tab_IXP[-1]), ' < ', int(tab_Internet[-1]),
                                      ' but num host IXP > num host Internet.')

                                if to_add not in Computation_announcements[IXP_in_world][ixp]['IXPShorterThanUpstream']:
                                    Computation_announcements[IXP_in_world][ixp]['IXPShorterThanUpstream'].append(
                                        to_add)
...
for IXP_in_world in list(Computation_announcements.keys()):
    ...
    for ixp in list(Computation_announcements[IXP_in_world].keys()):
        ...
        create_output.write(str(IXP_in_world) + '; IXPShorterThanUpstream; ' + str(
            len(list(set(Computation_announcements[IXP_in_world][ixp]['IXPShorterThanUpstream'])))) + '\n')

        create_output.write(str(IXP_in_world) + '; IXPLongerThanUpstream; ' + str(
            len(list(set(Computation_announcements[IXP_in_world][ixp]['IXPLongerThanUpstream'])))) + '\n')

        create_output.write(str(IXP_in_world) + '; IXPBalancedUpstream; ' + str(
            len(list(set(Computation_announcements[IXP_in_world][ixp]['IXPBalancedUpstream'])))) + '\n\n')
