...
## Follow the alert signs and make the suitable changes
...
Months = {'Jan': '01', 'Feb': '02', 'Mar': '03', 'Apr': '04', 'May': '05', 'June': '06', 'Jun': '06', 'Jul': '07',
          'July': '07', 'Aug': '08', 'Sept': '09', 'Sep': '09', 'Oct': '10', 'Nov': '11', 'Dec': '12'}

number_of_script_not_to_exceed = 1
...
directory = ['/home/arda/African_Route-collectors_Data_Analyzer-ARDA/ComputationVM/Heart/5_RegionalView/']
                        ...
                        for line in fh1:
                            ...
                            if 'started' in line:
                                ...
                                if len(tab[-3] + '/' + Months[tab[1]] + '/' + tab[-1]) == 10:
                                    ...
                                    cur.execute(
                                        """UPDATE Infos_on_all_scripts_to_run SET Last_end_execution_date=%s WHERE Script_to_execute=%s""",
                                        ('Unknown', marked_file))
                                ...
                                if len(Final_list_of_scripts[marked_file]['end_exec_date']) == 10:
                                    cur.execute(
                                        """UPDATE Infos_on_all_scripts_to_run SET Last_end_execution_date=%s WHERE Script_to_execute=%s""",
                                        (Final_list_of_scripts[marked_file]['end_exec_date'], marked_file))
...
os.chdir('/home/arda/African_Route-collectors_Data_Analyzer-ARDA/ComputationVM/Heart/7_scheduler')
...
if len(List_all_tables) == 0:
    ...
    for script in Final_list_of_scripts:
        ...
        print('', directory_add, script, launch_exec_date_add, end_exec_date_add, Best_execution_period_add,
              Worse_execution_period_add, Priority_queue_add)
        cur.execute(
            """INSERT INTO Infos_on_all_scripts_to_run (Item, Directory, Script_to_execute ,Last_beg_execution_date, Last_end_execution_date, Best_execution_period, Worse_execution_period,  Current_Status_machine, best_memory_usage, worse_memory_usage, best_cpu_usage, worse_cpu_usage,    Priority_queue) VALUES (%s, %s,  %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) """,
            ('', directory_add, script, launch_exec_date_add, end_exec_date_add, Best_execution_period_add,
             Worse_execution_period_add, Current_Status_machine_add, best_memory_usage_add, worse_memory_usage_add,
             best_cpu_usage_add, worse_cpu_usage_add, Priority_queue_add))
    ...
    for script in Final_list_of_scripts:
        ...
        # print  'LINE = ', "'"+ directory_add +"'", "'"+script+"'"
...
for row in cur.fetchall():
    ...
    if Priority_queue_add == 'Yes' and '5_RegionalView_' in script:
        Priority_queue_list[script] = directory_add

    Dict_all_scripts_to_run[script] = [id, '', directory_add, script, Last_beg_execution_date_add,
                                       Last_end_execution_date_add, Best_execution_period_add,
                                       Worse_execution_period_add, Status_running_add, Current_Status_machine_add,
                                       best_memory_usage_add, worse_memory_usage_add, best_cpu_usage_add,
                                       best_cpu_usage_add, worse_cpu_usage_add, pid_add, Priority_queue_add]
...
for row in processes[1:]:
    ...
    if 'python' in row:
        for script_to_run in list(Dict_all_scripts_to_run.keys()):
            if '5_RegionalView_' in script_to_run:
                print()
        ...
        for pid_script_currently_running in list(existing_python_scripts.keys()):
            ...
            if existing_python_scripts[pid_script_currently_running]['script_running'] == script_to_run:
                ...
                if float(current_best_memory_usage_add) != float(-1):
                    if float(existing_python_scripts[pid_script_currently_running]['mem']) < float(
                            current_best_memory_usage_add):
                ...
                if float(current_best_cpu_usage_add) != float(-1):
                    if float(existing_python_scripts[pid_script_currently_running]['cpu']) < float(
                            current_best_cpu_usage_add):
                ...
                if float(Dict_all_scripts_to_run[script_to_run][13]) != float(-1):
                    ...
                    if float(existing_python_scripts[pid_script_currently_running]['cpu']) > float(
                            current_worse_cpu_usage_add):
                        current_worse_cpu_usage_add = float(
                            existing_python_scripts[pid_script_currently_running]['cpu'])

                else:
                    current_worse_cpu_usage_add = float(existing_python_scripts[pid_script_currently_running]['cpu'])

                print('After =', Last_beg_execution_date_add, Status_running_add, Current_Status_machine_add,
                      current_best_memory_usage_add, current_worse_memory_usage_add, current_best_cpu_usage_add,
                      current_worse_cpu_usage_add, pid_add, script_to_run)
                cur.execute(
                    """UPDATE Infos_on_all_scripts_to_run SET Last_beg_execution_date=%s, Current_Status=%s, Current_Status_machine =%s,  best_memory_usage = %s, worse_memory_usage= %s, best_cpu_usage =%s, worse_cpu_usage = %s, pid =%s WHERE Script_to_execute=%s""",
                    (Last_beg_execution_date_add, Status_running_add, Current_Status_machine_add,
                     current_best_memory_usage_add, current_worse_memory_usage_add, current_best_cpu_usage_add,
                     current_worse_cpu_usage_add, pid_add, script_to_run))
...
for script_to_run in list(Dict_all_scripts_to_run.keys()):
    if '5_RegionalView_' in script_to_run:
        ...
        if indice_numb_case_non_equal == length:
            ...
            cur.execute(
                """UPDATE Infos_on_all_scripts_to_run SET  Current_Status=%s, Current_Status_machine = %s  WHERE Script_to_execute=%s""",
                (Status_running_add, '', script_to_run))

            ## Look for date end and compute period (best and worse)
            # print 'I am here'

            if Dict_all_scripts_to_run[script_to_run][4] != 'Unknown' and Dict_all_scripts_to_run[script_to_run][
                5] != 'Unknown':

                print('Current values = ', Dict_all_scripts_to_run[script_to_run][4],
                      Dict_all_scripts_to_run[script_to_run][5])
                ...
                if date_beg <= date_end:
                    ...
                    cur.execute(
                        """UPDATE Infos_on_all_scripts_to_run SET  Best_execution_period=%s, Worse_execution_period=%s  WHERE Script_to_execute=%s""",
                        (Best_execution_period_add, Worse_execution_period_add, script_to_run))
...
for row in cur.fetchall():
    ...
    print('Here are the date of today and 1 year ago ', date.today(), twelve_months_ago, 'Better written for update = ',
          date_a_year_ago)

    cur.execute(
        """UPDATE Infos_on_all_scripts_to_run SET Last_end_execution_date=%s WHERE Script_to_execute=%s and id=%s""",
        (date_a_year_ago, script, id))
...
if len(list(Priority_queue_list_copy.keys())) > 0:
    ...
    if number_of_script_running <= number_of_script_not_to_exceed:
        ...
        for script in Priority_queue_list:
            ...
            ## Alert: Replace Username by your username
            tab = CronTab(user='arda')

            direc = Priority_queue_list[script]
            os.chdir(direc)

            cmd = 'cd ' + direc + ' && ' + ' python3.8 ' + script
                        ...
                        if '5_RegionalView_' in script_to_run:
                            ...
                            if script_to_run in row:
                                if str(line_tab[-1]).strip() == 'python ' + str(script_to_run).strip():
                                    ...
                                    existing_python_scripts_now[line_tab[1]]['script_running'] = str(
                                        script_to_run).strip()
            ...
            for linee in existing_python_scripts_now:
                ...
                if script == existing_python_scripts_now[linee]['script_running']:
                        ...
                        for line in fh:
                            ...
                            if 'PID' not in line:
                                ...
                                if str(tab[0]).strip() in list(existing_python_scripts_now.keys()):
                                    ...
                                    print(line, 'date_found = ', date_found, ' Number of executions days = ',
                                          current_period_of_execution)

                                    existing_python_scripts_now[tab[0]]['date_launch'] = date_found
                                    existing_python_scripts_now[tab[0]][
                                        'current_period_exec'] = current_period_of_execution
                    ...
                    print(direc, Last_beg_execution_date_add, best_memory_usage_add, worse_memory_usage_add,
                          Status_running_add, best_cpu_usage_add, worse_cpu_usage_add, pid_add,
                          Current_Status_machine_add, Priority_queue_add)
                    ...
                    cur.execute(
                        """UPDATE Infos_on_all_scripts_to_run SET Last_beg_execution_date=%s, Current_Status=%s,  Current_Status_machine=%s,  best_memory_usage = %s, worse_memory_usage=%s, pid=%s, Priority_queue=%s, best_cpu_usage=%s, worse_cpu_usage=%s, Last_end_execution_date=%s WHERE Script_to_execute =%s""",
                        (Last_beg_execution_date_add, Status_running_add, Current_Status_machine_add,
                         best_memory_usage_add, worse_memory_usage_add, pid_add, Priority_queue_add, best_cpu_usage_add,
                         worse_cpu_usage_add, Last_end_execution_date_add, script))
