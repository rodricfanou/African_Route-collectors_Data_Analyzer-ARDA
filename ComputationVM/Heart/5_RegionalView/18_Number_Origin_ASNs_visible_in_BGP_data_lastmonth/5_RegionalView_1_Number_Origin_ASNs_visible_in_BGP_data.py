...
## Create a logfile:
name_log_file = 'Log_'+str(now_datetime) + '_' + '17_Number_Origin_ASNs_visible_in_BGP_data_lastmonth' + '.txt'
...
output_folder = '../../Computation_outputs_Regional_View/18_Number_Origin_ASNs_visible_in_BGP_data_lastmonth/'
...
if os.listdir(IXPView_output_folder) != []:
    ...
    for ixp in IXP_collector.keys():
        ...
        filename_peering =  'LastMonth__list_visible_ASNs_at_IXP_' + ixp + '.txt'
                ...
                for line in fd:
                    ...
                    if '###' not in line and len(tab) > 1:
                        ...
                        try:
                            ...
                            print  filename_peering ,'ASN = ', ASN
...
create_output = open(output_folder + 'LastMonth__infos_Origin_ASNs.txt', 'a')

print 'len(Unik_list_peering_ASNs) = ', len(Unik_list_peering_ASNs)


filename_output_ASNs = output_folder + 'List_Origin_ASNs_at_an_African_IXP.txt'

with open (filename_output_ASNs, 'a') as fg:
    for elmt in Unik_list_peering_ASNs:
        fg.write('%s\n' %(elmt))

create_output.write('Number of Origin ASNs visibles at an African IXP ; ' + str(len(Unik_list_peering_ASNs)) + ' \n')
...
print
print 'Percentage Origin ASNs = ', percentage
...
create_output.write('Number of Origin ASNs assigned by Afrinic visibles at an African IXP ; ' + str(len(Num_identical)) + ' \n')

create_output.write('Percentage Origin ASNs assigned by Afrinic visibles at an IXP ; ' + str(percentage) + ' \n')
