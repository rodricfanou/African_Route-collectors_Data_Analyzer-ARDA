...
name_log_file = 'Log_' + str(
    now_datetime) + '_' + 'RegionalView_1_Number_Origin_ASNs_visible_at_an_IXP_multiyear' + '.txt'
...
if os.listdir(IXPView_output_folder) != []:
    ...
    for Current_Year in List_years_clone:
        ...
        with open(output_folder + 'MultiYear__Total_Number_unique_Origin_ASNs_seen_at_1_IXP_and_more__' + str(
                Current_Year) + '.txt', 'w') as fgg:
        ...
        with open(output_folder + 'MultiYear__List_Origin_ASNs_0percent_20percent_Year_' + str(Current_Year) + '.txt',
                  'a') as fh:
            fh.write('%s\n' % ("""##ASN; IXPs at which it is visible"""))
            for elmt in Pie_chart__lastyear["0% to less than 20%"]:
                fh.write('%s; %s\n' % (elmt, '; '.join(Appear[elmt])))

        with open(output_folder + 'MultiYear__List_Origin_ASNs_20percent_40percent_Year_' + str(Current_Year) + '.txt',
                  'a') as fh:
            fh.write('%s\n' % ("""##ASN; IXPs at which it is visible"""))
            for elmt in Pie_chart__lastyear["20% to less than 40%"]:
                fh.write('%s; %s\n' % (elmt, '; '.join(Appear[elmt])))

        with open(output_folder + 'MultiYear__List_Origin_ASNs_40percent_60percent_' + str(Current_Year) + '.txt',
                  'a') as fh:
            fh.write('%s\n' % ("""##ASN; IXPs at which it is visible"""))
            for elmt in Pie_chart__lastyear["40% to less than 60%"]:
                fh.write('%s; %s\n' % (elmt, '; '.join(Appear[elmt])))

        with open(output_folder + 'MultiYear__List_Origin_ASNs_60percent_80percent_' + str(Current_Year) + '.txt',
                  'a') as fh:
            fh.write('%s \n' % ("""##ASN; IXPs at which it is visible"""))
            for elmt in Pie_chart__lastyear["60% to less than 80%"]:
                fh.write('%s; %s\n' % (elmt, '; '.join(Appear[elmt])))

        with open(output_folder + 'MultiYear__List_Origin_ASNs_80percent_100percent_' + str(Current_Year) + '.txt',
                  'a') as fh:
            fh.write('%s\n' % ("""##ASN; IXPs at which it is visible"""))
    ...
    create_output_2bytesASN_list = open(output_folder + 'MultiYear__2bytes_list_visible_ASNs_in_more_than_1_IXP.txt',
                                        'a')

    create_output_2bytesASN_list.write('###Year; Visible 2bytes Origin ASNs \n')

    create_output_2bytesASN_num = open(output_folder + 'MultiYear__2bytes_number_visible_ASNs_in_more_than_1_IXP.txt',
                                       'a')
    ...
    create_output_4bytesASN_list = open(output_folder + 'MultiYear__4bytes_list_visible_ASNs_in_more_than_1_IXP.txt',
                                        'a')

    create_output_4bytesASN_list.write('###Year; Visible 4bytes Origin ASNs \n')

    create_output_4bytesASN_num = open(output_folder + 'MultiYear__4bytes_number_visible_ASNs_in_more_than_1_IXP.txt',
                                       'a')
