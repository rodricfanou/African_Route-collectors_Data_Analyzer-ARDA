...
## Create a logfile:
name_log_file = 'Log_'+str(now_datetime) + '_' + 'RegionalView_1_Number_Origin_ASNs_visible_at_an_IXP_multiyear' + '.txt'
...
output_folder = '../../Computation_outputs_Regional_View/2_Number_Origin_ASNs_in_more_than_1_IXP_in_Region_multiyear/'
...
if os.listdir(IXPView_output_folder) != []:
    ...
    for Current_Year in List_years_clone:
        ...
        with open(output_folder + 'MultiYear__Total_Number_unique_Origin_ASNs_seen_at_1_IXP_and_more__'+ str(Current_Year) +'.txt', 'w') as fgg:
            fgg.write('%s\n' %('###Regional View -- The Total number of unique Origin ASNs seen at 1 IXP or more'))
            fgg.write('%s\n\n' %(len(Total_number_Origin_ASNs) ))
            
            fgg.write('%s\n' %('###Regional View -- Total number of IXPs'))
            fgg.write('%s\n\n' %( len(list(IXP_collector.keys()))  ))
            
            ## Improve he computation of the total number of IXP per year.
        ...
        with open (output_folder +'MultiYear__List_Origin_ASNs_0percent_20percent_Year_' + str(Current_Year)+ '.txt', 'a') as fh:
            fh.write('%s\n' %("""##ASN; IXPs at which it is visible"""))
            for elmt in Pie_chart__lastyear["0% to less than 20%"]:
                fh.write('%s; %s\n' %(elmt, '; '.join(Appear[elmt])))

        with open (output_folder +'MultiYear__List_Origin_ASNs_20percent_40percent_Year_' + str(Current_Year) + '.txt', 'a') as fh:
            fh.write('%s\n' %("""##ASN; IXPs at which it is visible"""))
            for elmt in Pie_chart__lastyear["20% to less than 40%"]:
                fh.write('%s; %s\n' %(elmt, '; '.join(Appear[elmt])))

        with open (output_folder +'MultiYear__List_Origin_ASNs_40percent_60percent_' + str(Current_Year) + '.txt', 'a') as fh:
            fh.write('%s\n' %("""##ASN; IXPs at which it is visible"""))
            for elmt in Pie_chart__lastyear["40% to less than 60%"]:
                fh.write('%s; %s\n' %(elmt, '; '.join(Appear[elmt])))

        with open (output_folder +'MultiYear__List_Origin_ASNs_60percent_80percent_' + str(Current_Year) + '.txt', 'a') as fh:
            fh.write('%s \n' %("""##ASN; IXPs at which it is visible"""))
            for elmt in Pie_chart__lastyear["60% to less than 80%"]:
                fh.write('%s; %s\n' %(elmt, '; '.join(Appear[elmt])))

        with open (output_folder +'MultiYear__List_Origin_ASNs_80percent_100percent_' + str(Current_Year) + '.txt', 'a') as fh:
            fh.write('%s\n' %("""##ASN; IXPs at which it is visible"""))
    ...
    create_output_2bytesASN_list =  open(output_folder+'MultiYear__2bytes_list_visible_ASNs_in_more_than_1_IXP.txt', 'a')
    
    create_output_2bytesASN_list.write('###Year; Visible 2bytes Origin ASNs \n')

    create_output_2bytesASN_num =  open(output_folder+'MultiYear__2bytes_number_visible_ASNs_in_more_than_1_IXP.txt', 'a')
    
    create_output_2bytesASN_num.write('###Year; Number Visible 2bytes Origin ASNs \n')
    ...
    create_output_4bytesASN_list =  open(output_folder+'MultiYear__4bytes_list_visible_ASNs_in_more_than_1_IXP.txt', 'a')
    
    create_output_4bytesASN_list.write('###Year; Visible 4bytes Origin ASNs \n')
    
    create_output_4bytesASN_num =  open(output_folder+'MultiYear__4bytes_number_visible_ASNs_in_more_than_1_IXP.txt', 'a')
    
    create_output_4bytesASN_num.write('###Year; Number Visible 4bytes Origin ASNs \n')
...
with open (output_folder + 'MultiYear__2and4bytes_number_visible_Origin_ASNs_in_more_than_1_IXP.txt', 'a') as fg:
    fg.write('%s\n' %("""###Year; Number of Visible Origin 2bytes ASNs; Number of Visible Origin 4bytes ASNs;"""))
