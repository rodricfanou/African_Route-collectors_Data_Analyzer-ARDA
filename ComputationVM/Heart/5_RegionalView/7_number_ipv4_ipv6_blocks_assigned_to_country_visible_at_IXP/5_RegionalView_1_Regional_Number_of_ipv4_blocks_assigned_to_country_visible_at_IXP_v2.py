...
name_log_file = 'Log_' + str(
    now_datetime) + '_' + '1_RegionalView_Number_of_ipv4_blocks_assigned_to_country_visible_at_IXP' + '.txt'
...
try:
    perc_IP_blocks_v4 = 100 * (
                float(len(List_prefixes_v4_all_cross_check)) / float(len(List_AF_prefixes_v4_all_cross_check)))
except:
    perc_IP_blocks_v4 = 0

try:
    perc_IP_blocks_v6 = 100 * (
                float(len(List_prefixes_v6_all_cross_check)) / float(len(List_AF_prefixes_v6_all_cross_check)))
except:
    perc_IP_blocks_v6 = 0
...
with open(output_folder + 'Results_percentages_v4_v6_prefixes.txt', 'a') as fdg:
    ...
    fdg.write('%s; %s; %s; %s \n' % (
    'Percentage of v4 prefixes attributed by AFRINIC seen at 1 or more IXP in Africa; ',
    str(len(List_prefixes_v4_all_cross_check)), str(len(List_AF_prefixes_v4_all_cross_check)), str(perc_IP_blocks_v4)))

    print(str(len(List_AF_prefixes_v6_all_cross_check)))

    fdg.write('%s; %s; %s; %s \n' % (
    'Percentage of v6 prefixes attributed by AFRINIC seen at 1 or more IXP in Africa; ',
    str(len(List_prefixes_v6_all_cross_check)), str(len(List_AF_prefixes_v6_all_cross_check)), str(perc_IP_blocks_v6)))
