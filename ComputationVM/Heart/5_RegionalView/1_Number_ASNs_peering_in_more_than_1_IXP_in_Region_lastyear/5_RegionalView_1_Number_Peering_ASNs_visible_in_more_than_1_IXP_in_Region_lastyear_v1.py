...
## Create a logfile:
name_log_file = 'Log_'+str(now_datetime) + '_' + 'RegionalView_1_Number_Peering_ASNs_visible_at_an_IXP_lastyear' + '.txt'
...
output_folder = '../../Computation_outputs_Regional_View/1_Number_ASNs_peering_in_more_than_1_IXP_in_Region_lastyear/'
...
if os.listdir(IXPView_output_folder) != []:
    ...
    for ixp in list(IXP_collector.keys()):
        ...
        filename = IXPView_output_folder+ """LastYear__list_visible_ASNs_peering_at_IXP_""" + ixp + '.txt'
        List_ASNs = []
    ...
    with open(output_folder + 'LastYear__Total_Number_unique_Peering_ASNs_seen_at_1_IXP_and_more.txt', 'w') as fgg:
    ...
    with open (output_folder +'LastYear__List_Peering_ASNs_0percent_20percent.txt', 'a') as fh:
        fh.write('%s\n' %("""##ASN; IXPs at which it is visible"""))
        for elmt in Pie_chart__lastyear["0% to less than 20%"]:
            fh.write('%s; %s\n' %(elmt, '; '.join(Appear[elmt])))


    with open (output_folder +'LastYear__List_Peering_ASNs_20percent_40percent.txt', 'a') as fh:
        fh.write('%s\n' %("""##ASN; IXPs at which it is visible"""))
        for elmt in Pie_chart__lastyear["20% to less than 40%"]:
            fh.write('%s; %s\n' %(elmt, '; '.join(Appear[elmt])))


    with open (output_folder +'LastYear__List_Peering_ASNs_40percent_60percent.txt', 'a') as fh:
        fh.write('%s\n' %("""##ASN; IXPs at which it is visible"""))
        for elmt in Pie_chart__lastyear["40% to less than 60%"]:
            fh.write('%s; %s\n' %(elmt, '; '.join(Appear[elmt])))


    with open (output_folder +'LastYear__List_Peering_ASNs_60percent_80percent.txt', 'a') as fh:
        fh.write('%s \n' %("""##ASN; IXPs at which it is visible"""))
        for elmt in Pie_chart__lastyear["60% to less than 80%"]:
            fh.write('%s; %s\n' %(elmt, '; '.join(Appear[elmt])))


    with open (output_folder +'LastYear__List_Peering_ASNs_80percent_100percent.txt', 'a') as fh:
        fh.write('%s\n' %("""##ASN; IXPs at which it is visible"""))
    ...
    create_output_2bytesASN_list =  open(output_folder+'LastYear__2bytes_list_visible_ASNs_peering_in_more_than_1_IXP.txt', 'a')
    
    create_output_2bytesASN_list.write('###Month-Year; Visible 2bytes Peering ASNs \n')

    create_output_2bytesASN_num =  open(output_folder+'LastYear__2bytes_number_visible_ASNs_peering_in_more_than_1_IXP.txt', 'a')
    
    create_output_2bytesASN_num.write('###Month-Year; Number Visible 2bytes Peering ASNs \n')

    for ixp in list(IXP_collector.keys()):
        
        filename = IXPView_output_folder+ """LastYear__2bytes_list_visible_ASNs_peering_at_IXP_""" + ixp + '.txt'
    ...
    create_output_4bytesASN_list =  open(output_folder+'LastYear__4bytes_list_visible_ASNs_peering_in_more_than_1_IXP.txt', 'a')
    
    create_output_4bytesASN_list.write('###Month-Year; Visible 4bytes Peering ASNs \n')
    
    create_output_4bytesASN_num =  open(output_folder+'LastYear__4bytes_number_visible_ASNs_peering_in_more_than_1_IXP.txt', 'a')
    
    create_output_4bytesASN_num.write('###Month-Year; Number Visible 4bytes Peering ASNs \n')



    for ixp in list(IXP_collector.keys()):
        
        filename = IXPView_output_folder+ """LastYear__4bytes_list_visible_ASNs_peering_at_IXP_""" + ixp + '.txt'
...
with open (output_folder + 'LastYear__2and4bytes_number_visible_Peering_ASNs_in_more_than_1_IXP.txt', 'a') as fg:
    fg.write('%s\n' %("""###Month-Year; Number of Visible Peering 2bytes ASNs; Number of Visible Peering 4bytes ASNs;"""))
