...
## Create a logfile:
name_log_file = 'Log_'+str(now_datetime) + '_' + 'RegionalView_1_Number_of_prefixes_visible_at_all_IXPs_lastyear' + '.txt'
...
if os.listdir(IXPView_output_folder) != []:
    ...
    month_bogon_prefixes = {}

    create_output_prefixes_list =  open(output_folder+'LastYear__list_prefixes_visible_at_all_IXPs.txt', 'a')
    
    create_output_prefixes_list.write('###Month-Year; Visible prefixes at all IXPs; Bogon ? \n')
                ...
                for line in fg:
                    ...
                    if "#" not in line:
                        ...
                        #try:
                            ...
                            if  prefix_to_add != '':
                                if prefix_to_add not in month_prefixes[key_timestamp]:
                                    month_prefixes[key_timestamp].append(prefix_to_add)

                            if  bogon_to_add != '':
                                if bogon_to_add not in month_bogon_prefixes[key_timestamp]:
                                    month_bogon_prefixes[key_timestamp].append(bogon_to_add)
...
with open (output_folder + 'LastYear__number_prefixes_visible_at_all_IXPs.txt', 'a') as fg:
