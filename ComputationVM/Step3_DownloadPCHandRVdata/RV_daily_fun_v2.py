    ...
    while (stream.get_next_record(rec)):
        ...
        if rec.status != "valid":
            ...
            while (elem):
                ...
                TypeRC, TypeRCid, Date, Timestamp, Date.year, Date.month, Date.day, route_collector, OriginAS, NextAS,
                NextHop, Network, ASPath, ASPathLength, IPversion))
                idx += 1
...
command = 'python costas_mail.py "roderick.fanou@gmail.com" "RouteViews download on ' + str(
    today) + '" "We have downloaded RouteViews data from ' + str(beg_date) + ' (' + str(beg_timestamp) + ') to ' + str(
    actual_date) + ' (' + str(actualTimeStamp) + ')\nWe have collected ' + str(lines[
                                                                                   'route-views.kixp']) + ' lines for KIXP collector\nWe have collected lines for JINX collector\nThe size of the DB is ' + str(
    size_of_DB) + ' MB"'
os.system(command)
