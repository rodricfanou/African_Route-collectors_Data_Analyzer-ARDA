    ...
    for rc in dirs:
        if rc != 'index.html':
            ...
            if task == 'check_insert':
                query = ("SELECT RouteCollector, Network, Timestamp, ASpath FROM Data__" + str(year) + "_" + str(
                    int(month)) + " where RouteCollector = %s;")
                a = [str(rc)]
                cur.execute(query, a)
                    ...
                    with gzip.open(file_to_process, 'r') as file_h:
                        ...
                        for entry_n, bgp_entry in enumerate(bgp_rib.BGPRIB.parse_cisco_show_ip_bgp_generator(file_h)):
                            ...
                            if bgp_entry[7] == 'i':
                                ...
                                logger.warning("Found invalid prefix at bgp entry %s, with content %s, on file %s" % (
                                entry_n, bgp_entry, file_to_process))
                            ...
                            if (task == 'insert_only'):
                                try:
                                    query = "insert ignore into Data__" + str(year) + "_" + str(
                                        int(month)) + " (TypeRC, TypeRCid, Date, Timestamp, Year, Month, Day, RouteCollector, OriginAS, NextAS, NextHop, Network, ASPath, ASpathlength, Origin, IP_version) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);"
                                    cur.execute(query,
                                                ['PCH', RouteCollectorID, datetime_format, timestamp_format, str(year),
                                                 str(month), str(day), str(rc), originas, nextas, next_hop, network,
                                                 full_as_path, as_path_length, origin, 'v4'])
                                    # print query
                                except:
                                ...
                                try:
                                    ...
                                    if (chunk not in Box_Data):
                                        query = "insert ignore into Data__" + str(year) + "_" + str(
                                            int(month)) + " (TypeRC, TypeRCid, Date, Timestamp, Year, Month, Day, RouteCollector, OriginAS, NextAS, NextHop, Network, ASPath, ASpathlength, Origin, IP_version) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);"
                                        cur.execute(query, ['PCH', RouteCollectorID, datetime_format, timestamp_format,
                                                            str(year), str(month), str(day), str(rc), originas, nextas,
                                                            next_hop, network, full_as_path, as_path_length, origin,
                                                            'v4'])
                                        # print query
                                except:
