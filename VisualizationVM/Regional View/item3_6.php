    ...
    <head>
        ...
        <?php
            ...
            for ($i=0; $i < count($listCC); $i++) {
                if (file_exists('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastmonth_each_country/LastMonth__Number_ASNs_announcing_v4_v6_at_IXPs_in_'.$listCC[$i][0].'.txt') == 0) {
                    $file = fopen('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastmonth_each_country/LastMonth__Number_ASNs_announcing_v4_v6_at_IXPs_in_'.$listCC[$i][0].'.txt', "w");
                    fwrite($file, "" . PHP_EOL);
                    fclose($file);
                } 
                $fileOpen = fopen('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastmonth_each_country/LastMonth__Number_ASNs_announcing_v4_v6_at_IXPs_in_'.$listCC[$i][0].'.txt','r');
                if (!$fileOpen){
                    echo 'ERROR: No ha sido posible abrir el archivo1'.$listCC[$i].'. Revisa su nombre y sus permisos.'; exit;
                }
                ...
                for ($j=0; $j < 5; $j++) {
                    for ($k=0; $k < count($list); $k++){
                        if (strcmp($list[$k][0],strval($j+1)) == 0) {
                            ...
                            $endDate = $endDate[0]; 
                            $auxi = ["Week ".$list[$k][0]."\n (".trim($initialDate)." to \n".trim($endDate).")", intval($list[$k][5]), intval($list[$k][6])];
                            array_push($list2, $auxi); 
                ...
                $lastMonthPrefixes[$i] = ($list2);
            }

         $lastYearPrefixes = array();
            for ($i=0; $i < count($listCC); $i++) {
                 if (file_exists('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastyear_each_country/LastYear__Number_ASNs_announcing_v4_v6_at_IXPs_in_'.$listCC[$i][0].'.txt') == 0) {
                    $file = fopen('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastyear_each_country/LastYear__Number_ASNs_announcing_v4_v6_at_IXPs_in_'.$listCC[$i][0].'.txt', "w");
                    fwrite($file, "" . PHP_EOL);
                    fclose($file);
                } 
                $fileOpen = fopen('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastyear_each_country/LastYear__Number_ASNs_announcing_v4_v6_at_IXPs_in_'.$listCC[$i][0].'.txt','r');
                if (!$fileOpen){
                    echo 'ERROR: No ha sido posible abrir el archivo2'.$listCC[$i].'. Revisa su nombre y sus permisos.'; exit;
                }
            ...
            for ($i=0; $i < count($listCC); $i++) { 
                 if (file_exists('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastyear_each_country/MultiYear__Number_ASNs_announcing_v4_v6_at_IXPs_in_'.$listCC[$i][0].'.txt') == 0) {
                    $file = fopen('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastyear_each_country/MultiYear__Number_ASNs_announcing_v4_v6_at_IXPs_in_'.$listCC[$i][0].'.txt', "w");
                    fwrite($file, "" . PHP_EOL);
                    fclose($file);
                } 
                $fileOpen = fopen('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastyear_each_country/MultiYear__Number_ASNs_announcing_v4_v6_at_IXPs_in_'.$listCC[$i][0].'.txt','r');
                if (!$fileOpen){
                    echo 'ERROR: No ha sido posible abrir el archivo3'.$listCC[$i].'. Revisa su nombre y sus permisos.'; exit;
                }
        ...
        <script type="text/javascript">
            ...
            function paint() {
                ...
                if (numero == 0) {
                    ...
                    titulo.innerHTML = "Number of ASNs announcing v4/v6 at "+listCC[0][0];
                    titulo2.innerHTML = "Number of ASNs announcing v4/v6 at "+listCC[0][0];
                    titulo3.innerHTML = "Number of ASNs announcing v4/v6 at "+listCC[0][0];
                }else{
                    drawChart(numero-1);
                    var titulo = document.getElementById("titulo1");
                    var titulo3 = document.getElementById("titulo2");
                    ...
                    titulo3.innerHTML = "Number of ASNs announcing v4/v6 at "+IXP;
                }
            }
           ...
           $(window).resize(function () {
            ...
            function descarga1(){
                ...
                var numero = seleccionado.selectedIndex;
                if (numero == 0) {
                    alert("Select an Country");
                }else{
                    IXP = (seleccionado.value).split(",")[0];
                    console.log (IXP)
                    downloadURI('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastmonth_each_country/LastMonth__list_ASNs_announcing_v4_at_IXPs_in_'+IXP+'.txt','LastMonth__list_ASNs_announcing_v4_at_'+IXP+'.txt');
                    downloadURI('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastmonth_each_country/LastMonth__list_ASNs_announcing_v6_at_IXPs_in_'+IXP+'.txt','LastMonth__list_ASNs_announcing_v6_at_'+IXP+'.txt');
                }
            ...
            function descarga2(){
                ...
                if (numero == 0) {
                    ...
                    IXP = (seleccionado.value).split(",")[0];
                    console.log (IXP)
                    downloadURI('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastyear_each_country/LastYear__list_ASNs_announcing_v4_at_IXPs_in_'+IXP+'.txt','LastYear__list_ASNs_announcing_v4_at_'+IXP+'.txt');
                    downloadURI('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastyear_each_country/LastYear__list_ASNs_announcing_v6_at_IXPs_in_'+IXP+'.txt','LastYear__list_ASNs_announcing_v6_at_'+IXP+'.txt');
                }
            ...
            function descarga3(){
                ...
                if (numero == 0) {
                    ...
                    IXP = (seleccionado.value).split(",")[0];
                    console.log (IXP)
                    downloadURI('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastyear_each_country/MultiYear__list_ASNs_announcing_v4_at_IXPs_in_'+IXP+'.txt','Multiyear__list_ASNs_announcing_v4_at_'+IXP+'.txt');
                    downloadURI('outputs_National_View/9_Number_IPv4_IPv6_prefixes_lastyear_each_country/MultiYear__list_ASNs_announcing_v6_at_IXPs_in_'+IXP+'.txt','Multiyear__list_ASNs_announcing_v6_at_'+IXP+'.txt');
                }
