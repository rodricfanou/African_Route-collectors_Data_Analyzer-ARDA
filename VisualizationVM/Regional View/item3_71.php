        ...
        <?php
            ...
            for ($i=0; $i < count($listCC); $i++) {
                ...
                for ($j=1; $j < $numero+1 ; $j++) { 
                    ...
                    $list12[$j-1][3] = 'Total number of ASNs visible at '.$listIXP[$i].': <b>'.number_format(floatval($list1[$j][3])).'</b>';
                    $list12[$j-1][4] = 'Number of ASNs assigned to '.$list1[$j][0].' by '.$list1[$j][7].' visible at '.$listIXP[$i].': <b>'.$list1[$j][2].' ('.(number_format(floatval($list1[$j][4]),2)).'%)</b>'.
                    '<br>% ASNs assigned to '.$list1[$j][0].' by '.$list1[$j][7].' visible at '.$listIXP[$i].': <b>'.(number_format(floatval($list1[$j][6]),2)).'%</b>';
