    ...
    <head>
        ...
        <?php
            ...
            for ($i=0; $i < count($listIXP); $i++) {
                if (file_exists('outputs/5_percentage_prefixes_by_country_assignment_lastmonth_better/Percentage_prefixes_by_region_'.$listIXP[$i][0].'.txt') == 0) {
                    $file = fopen('outputs/5_percentage_prefixes_by_country_assignment_lastmonth_better/Percentage_prefixes_by_region_'.$listIXP[$i][0].'.txt', "w");
                    fwrite($file, "" . PHP_EOL);
                    fclose($file);
                } 
                ...
                $fileOpen = fopen('outputs/5_percentage_prefixes_by_country_assignment_lastmonth_better/Percentage_prefixes_by_region_'.$listIXP[$i][0].'.txt','r');
                $numero = count(file('outputs/5_percentage_prefixes_by_country_assignment_lastmonth_better/Percentage_prefixes_by_region_'.$listIXP[$i][0].'.txt'));
                if (!$fileOpen){
            ...
            for ($i=0; $i < count($listIXP); $i++) {
                if (file_exists('outputs/5_percentage_prefixes_by_country_assignment_lastmonth_better/Percentage_prefixes_by_country_assignment_'.$listIXP[$i][0].'.txt') == 0) {
                    $file = fopen('outputs/5_percentage_prefixes_by_country_assignment_lastmonth_better/Percentage_prefixes_by_country_assignment_'.$listIXP[$i][0].'.txt', "w");
                    fwrite($file, "" . PHP_EOL);
                    fclose($file);
                }  
                ...
                $fileOpen = fopen('outputs/5_percentage_prefixes_by_country_assignment_lastmonth_better/Percentage_prefixes_by_country_assignment_'.$listIXP[$i][0].'.txt','r');
                $numero = count(file('outputs/5_percentage_prefixes_by_country_assignment_lastmonth_better/Percentage_prefixes_by_country_assignment_'.$listIXP[$i][0].'.txt'));
                if (!$fileOpen){
                ...
                //Txt in to an array
                $fileOpen = fopen('outputs/5_percentage_prefixes_by_country_assignment_lastmonth_better/Number_ASNs_assigned_by_Afrinic.txt','r');
                if (!$fileOpen){
                ...
                for ($j=1; $j < $numero+1 ; $j++) { 
                    ...
                    '<br>Number of Prefixes visible at '.$listIXP[$i][0].': <b>'.number_format(floatval($list1[$j][3])).'</b>'.
                    '<br>Number of Prefixes assigned to '.$list1[$j][0].' visible at '.$listIXP[$i][0].': <b>'.$list1[$j][2].' ('.(number_format(floatval($list1[$j][4]),2)).'%)</b>'.
                    '<br>% Prefixes assigned to '.$list1[$j][0].' visible at '.$listIXP[$i][0].': <b>'.(number_format(floatval($list1[$j][6]),2)).'%</b>'. 
                    '<br>Number of Prefixes assigned to '.$list1[$j][0].' by AFRINIC: <b>'.$number_AFRINIC[$list1[$j][0]].'</b>';
            ...
            $lastYearASNs = array();
            for ($i=0; $i < count($listIXP); $i++) {
                if (file_exists('outputs/5_percentage_prefixes_by_country_assignment_lastyear_better/LastYear__local_vs_External_prefixes_visible_at_IXP_'.$listIXP[$i][0].'.txt') == 0) {
                    $file = fopen('outputs/5_percentage_prefixes_by_country_assignment_lastyear_better/LastYear__local_vs_External_prefixes_visible_at_IXP_'.$listIXP[$i][0].'.txt', "w");
                    fwrite($file, "" . PHP_EOL);
                    fclose($file);
                }
                $fileOpen = fopen('outputs/5_percentage_prefixes_by_country_assignment_lastyear_better/LastYear__local_vs_External_prefixes_visible_at_IXP_'.$listIXP[$i][0].'.txt','r');
                if (!$fileOpen){
            ...
            $multiYearASNs = array();
            for ($i=0; $i < count($listIXP); $i++) {
                if (file_exists('outputs/5_percentage_prefixes_by_country_assignment_lastyear_better/MultiYear__local_vs_External_prefixes_visible_at_IXP_'.$listIXP[$i][0].'.txt') == 0) {
                    $file = fopen('outputs/5_percentage_prefixes_by_country_assignment_lastyear_better/MultiYear__local_vs_External_prefixes_visible_at_IXP_'.$listIXP[$i][0].'.txt', "w");
                    fwrite($file, "" . PHP_EOL);
                    fclose($file);
                } 
                $fileOpen = fopen('outputs/5_percentage_prefixes_by_country_assignment_lastyear_better/MultiYear__local_vs_External_prefixes_visible_at_IXP_'.$listIXP[$i][0].'.txt','r');
                if (!$fileOpen){
                    echo 'ERROR: No ha sido posible abrir el archivo'.$listIXP[$i][0].'. Revisa su nombre y sus permisos.'; exit;
                }
                ...
                for ($j=0; $j < intval(date("Y"))-2004; $j++) {
                    $list2[$j][0] = $list[$j][2];
                    $list2[$j][1] = intval($list[$j][4]);
                    $list2[$j][2] = intval($list[$j][3]);
                }
                $list = $list2;
        ...
        <script type="text/javascript">
            ...
            function init(){
                ...
                titulo.innerHTML = "Percentage of Prefixes allocated to "+listIXP[0][1]+" visible or not at "+listIXP[0][0];
                titulo2.innerHTML = "Percentage of Prefixes assigned by each RIR visible at "+listIXP[0][0];
                titulo3.innerHTML = "Percentage of Prefixes assigned to each country by its corresponding RIR";
                titulo4.innerHTML = "Local vs External Prefixes visible at "+listIXP[0][0];
                titulo5.innerHTML = "Local vs External Prefixes visible at "+listIXP[0][0];
                titulo6.innerHTML = "Visible Prefixes at "+listIXP[0][0]+" over the last month";
                formatoCorrecto();
                drawChart(0);
                drawRegionsMap(0);
                console.log("hola")
            ...
            function paint(){
                ...
                var titulo5 = document.getElementById("titulo5");
                var titulo6 = document.getElementById("subtitulo3");
                titulo.innerHTML = "Percentage of Prefixes allocated to "+listIXP[numero-1][1]+" visible or not at "+listIXP[numero-1][0];
                titulo2.innerHTML = "Percentage of Prefixess assigned by each RIR visible at "+listIXP[numero-1][0];
                titulo4.innerHTML = "Local vs External Prefixes visible at "+listIXP[numero-1][0];
                titulo5.innerHTML = "Local vs External Prefixes visible at "+listIXP[numero-1][0];
                titulo6.innerHTML = "Visible Prefixes at "+listIXP[numero-1][0]+" over the last month";
                drawChart(numero-1);
            ...
            function drawChart(i) {
                ...
                chart2.draw(data2, options2);
                 function selectHandler2() {
                    ...
                    if (confirm("Do you want to download this file?") == true) {
                        downloadURI('outputs/5_percentage_prefixes_by_country_assignment_lastmonth/files_prefixes/'+value+'_'+listIXP[i][0]+'.txt',value+'_'+listIXP[i][0]+'.txt');
                    }
            ...
            function descarga3(){
                ...
                if (numero == 0) {
                    ...
                    console.log (IXP)
                    downloadURI('outputs/5_percentage_prefixes_by_country_assignment_lastyear_better/MultiYear__local_vs_External_prefixes_visible_at_IXP_'+IXP+'.txt',seleccionado.value+'.txt');
                }
            ...
            function descarga2(){
                ...
                if (numero == 0) {
                    ...
                    IXP = (seleccionado.value).split(",")[0];
                    console.log (IXP)
                    downloadURI('outputs/5_percentage_prefixes_by_country_assignment_lastyear_better/LastYear__local_vs_External_prefixes_visible_at_IXP_'+IXP+'.txt',seleccionado.value+'.txt');
                }
