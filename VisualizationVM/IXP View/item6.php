<html>
    <head>
        ...
        <?php
            ...
            //Origin
            $lastMonthASNs_v4 = array();
            for ($i=0; $i < count($listIXP); $i++) { 
                if (file_exists('outputs/9_Number_IPv6_vs_IPv4_prefixes_lastmonth/LastMonth_'.$listIXP[$i][0].'_Number_ASNs_announcing_v4.txt') == 0) {
                    $file = fopen('outputs/9_Number_IPv6_vs_IPv4_prefixes_lastmonth/LastMonth_'.$listIXP[$i][0].'_Number_ASNs_announcing_v4.txt', "w");
                    fwrite($file, "" . PHP_EOL);
                    fclose($file);
                } 
                $fileOpen = fopen('outputs/9_Number_IPv6_vs_IPv4_prefixes_lastmonth/LastMonth_'.$listIXP[$i][0].'_Number_ASNs_announcing_v4.txt','r');
                if (!$fileOpen){
                ...
                $lastMonthASNs_v4[$i] = $list;
            }

             //Origin
            $lastMonthASNs_v6 = array();
            for ($i=0; $i < count($listIXP); $i++) { 
                if (file_exists('outputs/9_Number_IPv6_vs_IPv4_prefixes_lastmonth/LastMonth_'.$listIXP[$i][0].'_Number_ASNs_announcing_v6.txt') == 0) {
                    $file = fopen('outputs/9_Number_IPv6_vs_IPv4_prefixes_lastmonth/LastMonth_'.$listIXP[$i][0].'_Number_ASNs_announcing_v6.txt', "w");
                    fwrite($file, "" . PHP_EOL);
                    fclose($file);
                } 
                $fileOpen = fopen('outputs/9_Number_IPv6_vs_IPv4_prefixes_lastmonth/LastMonth_'.$listIXP[$i][0].'_Number_ASNs_announcing_v6.txt','r');
                if (!$fileOpen){
                    echo 'ERROR: No ha sido posible abrir el archivo'.$listIXP[$i][0].'. Revisa su nombre y sus permisos.'; exit;
                }
                ...
                $lastMonthASNs_v6[$i] = $list;
            }

            $lastYearASNsv4 = array();
            for ($i=0; $i < count($listIXP); $i++) {
                if (file_exists('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/LastYear_'.$listIXP[$i][0].'_Number_ASNs_announcing_v4.txt') == 0) {
                    $file = fopen('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/LastYear_'.$listIXP[$i][0].'_Number_ASNs_announcing_v4.txt', "w");
                    fwrite($file, "" . PHP_EOL);
                    fclose($file);
                }
                $fileOpen = fopen('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/LastYear_'.$listIXP[$i][0].'_Number_ASNs_announcing_v4.txt','r');
                if (!$fileOpen){
                    echo 'ERROR: No ha sido posible abrir el archivo '.$listIXP[$i][0].'. Revisa su nombre y sus permisos.'; exit;
                }
            ...
            for ($i=0; $i < count($listIXP); $i++) {
                if (file_exists('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/LastYear_'.$listIXP[$i][0].'_Number_ASNs_announcing_v6.txt') == 0) {
                    $file = fopen('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/LastYear_'.$listIXP[$i][0].'_Number_ASNs_announcing_v6.txt', "w");
                    fwrite($file, "" . PHP_EOL);
                    fclose($file);
                }
                $fileOpen = fopen('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/LastYear_'.$listIXP[$i][0].'_Number_ASNs_announcing_v6.txt','r');
                if (!$fileOpen){
                    echo 'ERROR: No ha sido posible abrir el archivo '.$listIXP[$i][0].'. Revisa su nombre y sus permisos.'; exit;
                }
            ...
            for ($i=0; $i < count($listIXP); $i++) {
                if (file_exists('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/MultiYear_'.$listIXP[$i][0].'_Number_ASNs_announcing_v4.txt') == 0) {
                    $file = fopen('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/MultiYear_'.$listIXP[$i][0].'_Number_ASNs_announcing_v4.txt', "w");
                    fwrite($file, "" . PHP_EOL);
                    fclose($file);
                } 
                $fileOpen = fopen('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/MultiYear_'.$listIXP[$i][0].'_Number_ASNs_announcing_v4.txt','r');
                if (!$fileOpen){
            ...
            for ($i=0; $i < count($listIXP); $i++) {
                if (file_exists('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/MultiYear_'.$listIXP[$i][0].'_Number_ASNs_announcing_v6.txt') == 0) {
                    $file = fopen('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/MultiYear_'.$listIXP[$i][0].'_Number_ASNs_announcing_v6.txt', "w");
                    fwrite($file, "" . PHP_EOL);
                    fclose($file);
                } 
                $fileOpen = fopen('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/MultiYear_'.$listIXP[$i][0].'_Number_ASNs_announcing_v6.txt','r');
                if (!$fileOpen){
                    echo 'ERROR: No ha sido posible abrir elkjhkjh archivo '.$listIXP[$i][0].'. Revisa su nombre y sus permisos.'; exit;
                }
                ...
                while (!feof($fileOpen)) { // loop hasta que se llegue al final del archivo
                    $line = trim(fgets($fileOpen)); // guardamos toda la línea en $line como un string
                    $list[$index] = explode(";",$line);
                    $list[$index][1] = trim($list[$index][1]);
                    $fileOpen++; // necesitamos llevar el puntero del archivo a la siguiente línea
                    $index++;
                }
                for ($k=0; $k < count($plantilla); $k++) { 
        ...
        <script type="text/javascript">
            ...
            var lastMonthASNs_v4 = (<?php echo json_encode($lastMonthASNs_v4)?>);
            var lastMonthASNs_v6 = (<?php echo json_encode($lastMonthASNs_v6)?>);
            ...
            for (var i = 0; i < listIXP.length; i++) {
                aux = []
                for (var j = 0; j < lastMonthASNs_v4[i].length; j++) {
                    aux[j] = [lastMonthASNs_v4[i][j][0], parseInt(lastMonthASNs_v4[i][j][1]), parseInt(lastMonthASNs_v6[i][j][1])]
               };
               porcentaje3.push(aux);
            };
            ...
            function drawChart(i) {
                ...
                var chart = new google.visualization.SteppedAreaChart(document.getElementById('origin'));
           ...
           $(window).resize(function () {
            ...
            function descarga1(){
                ...
                if (numero == 0) {
                    ...
                    IXP = (seleccionado.value).split(",")[0];
                    console.log (IXP)
                    downloadURI('outputs/9_Number_IPv6_vs_IPv4_prefixes_lastmonth/LastMonth_'+IXP+'_List_ASNs_announcing_v4.txt','Last_5_week_number_of_ASNs_announcing_v4_prefixes_at_'+IXP+'_raw_data.txt');
                    downloadURI('outputs/9_Number_IPv6_vs_IPv4_prefixes_lastmonth/LastMonth_'+IXP+'_List_ASNs_announcing_v6.txt','Last_5_week_number_of_ASNs_announcing_v6_prefixes_at_'+IXP+'_raw_data.txt');
                }
            ...
            function descarga2(){
                ...
                if (numero == 0) {
                    ...
                    IXP = (seleccionado.value).split(",")[0];
                    console.log (IXP)
                    downloadURI('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/LastYear_'+IXP+'_List_ASNs_announcing_v4.txt','Last_12_months_number_of_ASNs_announcing_v4_prefixes_at_'+IXP+'_raw_data.txt');
                    downloadURI('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/LastYear_'+IXP+'_List_ASNs_announcing_v6.txt','Last_12_months_number_of_ASNs_announcing_v6_prefixes_at_'+IXP+'_raw_data.txt');
                }
            ...
            function descarga3(){
                ...
                if (numero == 0) {
                    ...
                    IXP = (seleccionado.value).split(",")[0];
                    console.log (IXP)
                    downloadURI('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/MultiYear_'+IXP+'_List_ASNs_announcing_v4.txt','Multiyear_number_of_ASNs_announcing_v4_prefixes_at_'+IXP+'_raw_data.txt');
                    downloadURI('outputs/9_Number_IPv6_vs_IPv4_prefixes_multiyear/MultiYear_'+IXP+'_List_ASNs_announcing_v6.txt','Multiyear_number_of_ASNs_announcing_v4_prefixes_at_'+IXP+'_raw_data.txt');
                }
    ...
    <body>
        <div id="content">
            ...
            <div>
                <div id="parrafos">
                    <p class ="titulo" id="titulo3">Number of ASNs announcing v4/v6 at the IXP</p>     
                    <p id="subtitulo">Per week over the last month</p>
                </div>
                <div id="origin" style="width: 1000px; height: 300px;"></div>
